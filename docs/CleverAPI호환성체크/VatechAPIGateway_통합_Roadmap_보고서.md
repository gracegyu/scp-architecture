...
> 케이스 B·C 공통: 1·2단계는 작업 영역이 독립적이라 **동시 착수** 가능하고, GW 본체 **빌드도 병렬로 시작**할 수 있다. 다만 GW로의 **전환**(cutover)은 1·2가 안착한 뒤에 한다. 케이스 C는 단일 Region → 멀티 Region 재작업을 피하려고 **GW를 처음부터 멀티리전-ready로** 구축하는 최속 시나리오다(인력·멀티리전 확정이 전제).
...
| ② VatechAPIGateway 구축 | 2+3+4단계 | presigned·GW 일원화·멀티 Region을 한 스펙으로. 일정에 단계별 구현·병행/통합(케이스 C) 기술 |
| ③ Straumann(AXS) 연동 | 별도 트랙 | 사업 동인·리스크가 상이, 3단계 이후 착수 |

> 2+3+4를 한 스펙으로 묶으면 케이스 A·B·C(순차·병행·3·4 통합)를 모두 포괄한다.

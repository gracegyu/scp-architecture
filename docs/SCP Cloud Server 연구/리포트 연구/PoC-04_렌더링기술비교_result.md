 ...
 * @returns SVG path의 d 속성 값
 */
function bSplineToSVGPath(controlPoints: Point[], degree: number = 3, knots?: number[]): string {
  if (controlPoints.length < degree + 1) {
    // 점이 부족하면 직선으로 연결
    return controlPoints.map((p, i) => (i === 0 ? `M ${p.x},${p.y}` : `L ${p.x},${p.y}`)).join(' ')
  }
  ...
  for (let i = degree; i < controlPoints.length; i++) {
    ...
    const bezierPoints = convertBSplineSegmentToBezier(controlPoints, knots, degree, i)
...
1. **PoC-05 (외부 라이브러리 평가)**: SVG 조작 라이브러리 선정
2. **PoC-14 (Element 렌더링 엔진)**: HTML DOM+SVG 기반 렌더링 엔진 구현
3. **PoC-13 (접근성 준수)**: WCAG 2.1 AA 준수 검증

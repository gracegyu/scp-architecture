...
## 환자 활동성 기반 차등 라이프사이클(요구사항)

- 문제의식: 동일한 “3년 전 영상”이라도 환자의 최근 방문 빈도/재방문 가능성에 따라 조회 가능성이 크게 다르다.
- 요구사항: 환자 활동성(예: `lastVisitAt`, `visitCountIn24M`, `activeTreatmentPlan` 등)에 따라 아카이브 전환 시점/경로를 차등 적용한다.
- 구현 아이디어(정책 태깅): 객체 태그에 `patientActivity=active|inactive|mid` 등을 부여하여, 활동성이 높은 환자는 IA 유지 기간을 늘리고 Glacier 전환을 지연한다.
- 운영 가드레일: 의료 규정/보존 의무를 우선하고, 클리닉별 옵트인/옵트아웃 가능하게 한다(정책 템플릿 + 예외 태그).
- 추후 작업: 활동성 산출 로직 정의, 태깅 자동화(업로드/정기 배치), 비용/효과 검증 후 임계값 조정.

...
- 미디어 파일 저장소: 파일시스템 vs RocksDB vs MinIO
- 메타데이터 저장소: PostgreSQL vs Redis vs RocksDB
- 하이브리드 아키텍처: 최적 조합 검증
- 클라우드 호환성: MinIO ↔ AWS S3 동기화
...
1. **파일시스템이 순수 로컬 캐시에 최적**: 치과 이미지(1-50MB)는 파일시스템의 장점을 극대화하며, OS 캐시 활용과 백업 용이성에서 압도적 우위
...
4. **MinIO는 클라우드 하이브리드 시나리오에 유용**: S3 호환성과 동기화 기능으로 멀티 사이트 및 재해 복구에 적합하나, 순수 로컬 캐시에서는 HTTP 오버헤드로 성능 저하
...
| MinIO      | 225 MB/s           | 0.85 ms        | 2.1 ms        | 200 MB        |

**분석:**

- 파일시스템이 쓰기 처리량 44% 우수 (OS 버퍼 캐시 효과)
- RocksDB는 압축으로 디스크 사용량 17% 절감하나, 쓰기 지연 증가
- MinIO는 HTTP 오버헤드로 파일시스템 대비 21% 느림, RocksDB보다는 14% 빠름
...
| MinIO      | 340 MB/s           | 58.8 ms        | 105 ms        | 2,000 MB      |

**분석:**

- 대용량 파일에서 파일시스템이 압도적 우위 (약 3배 빠름)
- RocksDB는 큰 값(large value)에 비효율적 (Write Amplification 증가)
- MinIO는 파일시스템 대비 19% 느림 (HTTP 멀티파트 업로드 오버헤드), 하지만 RocksDB보다 2.3배 빠름
...
| MinIO      | 3,600          | 2.1 ms        | 5.5 ms        | 10.2 ms       | 90%         |
...
| MinIO      | 45             | 21.5 ms       | 45 ms         | 78 ms         |

**분석:**

- 작은 파일: RocksDB가 블록 캐시로 약간 우수 (9% 빠름), MinIO는 HTTP 오버헤드로 파일시스템 대비 6% 느림
- 큰 파일: 파일시스템이 OS 캐시 활용으로 압도적 우위 (약 2배 빠름), MinIO는 파일시스템 대비 13% 느림
- MinIO는 멀티파트 다운로드로 큰 파일에서 상대적으로 선방
...
- MinIO는 파일시스템 대비 17% 느림, HTTP 파이프라이닝으로 순차 읽기 최적화
- RocksDB는 압축 해제 오버헤드로 가장 느림
...
- MinIO는 Go 런타임과 HTTP 서버로 280MB 사용 (데이터 10GB 기준)

### 3.4 운영 복잡도

| 항목      | 파일시스템     | RocksDB     | MinIO                   |
| --------- | -------------- | ----------- | ----------------------- |
| 백업      | rsync 가능     | Export 필요 | mc mirror (S3 동기화)   |
| 복구      | 단순 복사      | Import 필요 | S3에서 자동 복구        |
| 디버깅    | 파일 직접 확인 | rocksdb CLI | mc ls, Web Console      |
| 모니터링  | df, du         | 별도 도구   | Prometheus metrics 내장 |
| 압축 튜닝 | 불필요         | 필수        | 불필요 (자동)           |
| 서비스    | 불필요         | 불필요      | 별도 프로세스 (systemd) |
...
- 멀티 사이트/클리닉 간 데이터 동기화 필요 시
- AWS S3와의 하이브리드 아키텍처 구축 시
- 재해 복구 자동화 (S3 백업) 필요 시
- 클라우드 마이그레이션 계획이 있는 경우

---

## 3.6 MinIO 클라우드 호환성 검증

### 3.6.1 S3 API 호환성 테스트

**테스트 도구:**

- AWS SDK (boto3, aws-sdk-s3)
- MinIO Client (mc)
- S3 Browser, Cyberduck

**호환성 결과:**

| S3 API 기능         | MinIO 지원 | 테스트 결과 | 비고                 |
| ------------------- | ---------- | ----------- | -------------------- |
| PutObject           | ✅         | 100% 호환   | 멀티파트 업로드 포함 |
| GetObject           | ✅         | 100% 호환   | Range 요청 지원      |
| DeleteObject        | ✅         | 100% 호환   | 배치 삭제 지원       |
| ListObjects         | ✅         | 100% 호환   | 페이지네이션 포함    |
| HeadObject          | ✅         | 100% 호환   | 메타데이터 조회      |
| CopyObject          | ✅         | 100% 호환   | 서버 측 복사         |
| Object Metadata     | ✅         | 100% 호환   | x-amz-meta-\* 지원   |
| Lifecycle Policy    | ✅         | 100% 호환   | 만료 정책 동작       |
| Versioning          | ✅         | 100% 호환   | 객체 버전 관리       |
| Bucket Policy       | ✅         | 100% 호환   | IAM 정책 지원        |
| Presigned URL       | ✅         | 100% 호환   | 임시 접근 URL        |
| Server-Side Encrypt | ✅         | 100% 호환   | AES-256              |

**결론:** MinIO는 S3 API와 100% 호환, boto3 및 aws-sdk로 코드 변경 없이 사용 가능

### 3.6.2 MinIO ↔ AWS S3 동기화 테스트

**동기화 도구:** MinIO Client (mc mirror)

**테스트 시나리오:**

```bash
# MinIO → S3 동기화
mc mirror local-minio/scp-cache-clinic123 aws-s3/scp-backup-clinic123

# S3 → MinIO 복구
mc mirror aws-s3/scp-backup-clinic123 local-minio/scp-cache-clinic123
```

**동기화 성능:**

| 데이터 크기 | 파일 수 | 동기화 속도 | 소요 시간 | 비고                   |
| ----------- | ------- | ----------- | --------- | ---------------------- |
| 10 GB       | 1,000   | 85 MB/s     | 2분       | 초기 동기화            |
| 1 GB        | 100     | 92 MB/s     | 11초      | 증분 동기화 (변경분만) |
| 50 GB       | 5,000   | 78 MB/s     | 11분      | 대용량 동기화          |

**분석:**

- 동기화 속도 78-92 MB/s로 목표치(> 10MB/s) 충분히 달성
- 증분 동기화로 네트워크 비용 절감 (변경된 파일만 전송)
- 자동화 스크립트(cron)로 주기적 백업 가능
...
- 10개 클리닉별 독립 버킷
- 동시 읽기/쓰기 부하 (클라이언트 50개)

**격리 성능:**

| 메트릭        | 결과        | 비고                          |
| ------------- | ----------- | ----------------------------- |
| 버킷 간 간섭  | 없음        | 각 버킷 독립 성능 유지        |
| IAM 정책 동작 | 정상        | 클리닉별 접근 제어 확인       |
| 동시 처리량   | 3,200 req/s | 단일 버킷 대비 성능 저하 없음 |
| 메모리 사용량 | 320 MB      | 버킷 추가 시 40MB 증가        |

**분석:**

- 멀티테넌시 구조로 클리닉별 데이터 격리 완벽
- 버킷 수 증가해도 성능 저하 미미 (선형 확장)

### 3.6.4 재해 복구 테스트

**시나리오:** MinIO 서버 장애 → S3 백업에서 복구

**복구 절차:**

1. 새 MinIO 서버 설치 (5분)
2. S3에서 데이터 복구 (mc mirror, 소요: 11분, 50GB 기준)
3. PostgreSQL 메타데이터 재구성 (2분)

**총 복구 시간:** 18분 (50GB 데이터 기준)

**분석:**

- S3 백업으로 빠른 재해 복구 가능
- RTO(Recovery Time Objective) 20분 이내 달성
- 자동화 스크립트로 복구 시간 단축 가능
...
**시나리오 B: 클라우드 하이브리드 (멀티 사이트/백업)**

**모든 규모:**

```
MinIO + PostgreSQL + Redis (선택) ↔ AWS S3
```
...
| 요구사항                   | 권장 구성               | 이유                 |
| -------------------------- | ----------------------- | -------------------- |
| 단일 클리닉, 순수 로컬     | 파일시스템 + PostgreSQL | 최고 성능, 운영 단순 |
| 멀티 사이트 동기화         | MinIO + PostgreSQL      | S3 호환, 자동 동기화 |
| 재해 복구 자동화           | MinIO + S3              | S3 백업, RTO 20분    |
| 클라우드 마이그레이션 대비 | MinIO + S3              | 점진적 클라우드 전환 |
| 임베디드 환경 (작은 파일)  | RocksDB                 | 단일 바이너리, 압축  |
...
- S3 API 100% 호환 (코드 변경 없이 AWS 전환 가능)
- 클라우드 동기화 속도 우수 (78-92 MB/s)
- 멀티테넌시 지원 (클리닉별 격리)
- 재해 복구 자동화 (RTO 20분 이내)

**단점:**

- HTTP 오버헤드로 순수 로컬 성능 13-21% 저하
- 별도 서비스 관리 필요 (메모리 280MB)
- 단일 클리닉에서는 과한 아키텍처

**결론:** 클라우드 하이브리드가 필요한 경우에만 사용
...
**작성일**: 2025-11-18  
**작성자**: Raymond  
**상태**: 검증 완료 - 파일시스템 + PostgreSQL 선정 (순수 로컬), MinIO + S3 (클라우드 하이브리드)

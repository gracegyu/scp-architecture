...
- [ ] 실제 접근 패턴 데이터 수집/분석(익명화, Zipf 계수/크기 분포 산출)
- [ ] 시뮬레이터 구현(LRU/SLRU/TinyLFU/W-TinyLFU 플러그 가능 구조)
- [ ] 알고리즘별 시뮬레이션/측정(요청/바이트 히트율, 메모리 OH, 지연)
- [ ] 프리페칭 전략 검증(환자/슬라이스/시간/예측 기반, 오버헤드 포함)
- [ ] 파라미터 튜닝 및 보고서(임계치/세그먼트 비율, 효과/복잡도 트레이드오프)

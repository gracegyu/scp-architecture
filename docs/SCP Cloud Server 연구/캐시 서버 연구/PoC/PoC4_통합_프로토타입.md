...
- [x] **Phase 4: 장애 대응** (2일, Week 3)

  - 목표: 폴백 메커니즘, 오프라인 모드, 재동기화
  - [x] 헬스체크 구현 (업스트림 서버 상태 모니터링)
  - [x] 장애 감지 로직 (3회 연속 실패 시)
  - [x] 클라이언트 폴백 메커니즘 (URI 경로 유지)
  - [x] 네트워크 단절 감지
  - [x] 오프라인 모드 구현 (로컬 캐시만 서빙)
  - [x] 오프라인 상태 UI 표시 지원
  - [x] 저널 재생 절차 구현
  - [x] 스풀 큐 재처리 절차
  - [x] 차등 스캔 구현 (`updatedAt` 비교)
  - [x] 재동기화 후 캐시 무효화

- [x] **Phase 5: 운영 준비** (2일, Week 3)

  - 목표: 인증/권한 처리, 보안 강화, 모니터링/로깅
  - [x] JWT 토큰 패스스루 (Authorization 헤더)
  - [x] 권한 변경 시 캐시 무효화 (Webhook 수신 준비)
  - [x] 디스크 암호화 (파일 단위 AES-256)
  - [x] 감사 로그 구현 (모든 접근 기록)
  - [x] 헬스체크 API (`GET /health`)
  - [x] 메트릭 API (`GET /metrics` - Prometheus 포맷)
  - [x] 수동 무효화 API (`POST /api/cache/invalidate`)
  - [x] 캐시 통계 API (`GET /api/cache/stats`)
  - [x] 구조화 JSON 액세스 로그
  - [x] Prometheus exporter 구현
  - [x] Windows Service 등록 (windows-service 크레이트)

- [x] **Phase 6: 성능 검증 및 최종 보고서** (1일, Week 3)
  - 목표: 부하 테스트, 성능 벤치마크, 최종 보고서 작성
  - [x] 정상 운영 시나리오 부하 테스트 스크립트 작성 (100명, 5,000 req/s, 1시간)
  - [x] 피크 부하 테스트 스크립트 작성 (300명, 10,000 req/s, 10분)
  - [x] 대용량 파일 테스트 준비 (100MB 파일, 동시 10건)
  - [x] 성능 벤치마크 도구 준비
  - [x] 장애 시나리오 테스트 준비 (네트워크 단절, 서버 재시작, 디스크 풀)
  - [x] 보안 테스트 준비 (인증 우회, 권한 없는 접근)
  - [x] 최종 보고서 작성 (PoC4*통합*프로토타입\_result.md 업데이트)

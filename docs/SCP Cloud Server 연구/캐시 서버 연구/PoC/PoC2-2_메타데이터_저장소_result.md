...
- 메타데이터 쓰기: > 1,000 ops/s
- 메타데이터 조회: > 5,000 ops/s, 95p < 10ms
- 복잡한 쿼리: 95p < 50ms
- TTL 만료 처리: 1,000건 < 1초
...
| 저장소     | 쓰기 처리량 (ops/s) | 평균 지연 (ms) | 95p 지연 (ms) | 99p 지연 (ms) |
| ---------- | ------------------- | -------------- | ------------- | ------------- |
| PostgreSQL | 1,850               | 5.41           | 12.35         | 18.92         |
| MongoDB    | 2,340               | 4.27           | 9.88          | 15.21         |
| Redis      | 8,920               | 1.12           | 2.45          | 3.87          |

**분석:**

- Redis가 **4.8배 빠름** (인메모리)
- MongoDB가 PostgreSQL보다 **26% 빠름** (BSON 네이티브)
- 모두 목표 (1,000 ops/s) 초과 달성 ✅

#### 시나리오 2: 단순 조회 (cache_key 기준, 10,000회)
...
| 저장소     | 조회 처리량 (ops/s) | 평균 지연 (ms) | 95p 지연 (ms) | 99p 지연 (ms) |
| ---------- | ------------------- | -------------- | ------------- | ------------- |
| PostgreSQL | 6,240               | 1.60           | 3.82          | 5.91          |
| MongoDB    | 7,810               | 1.28           | 3.01          | 4.67          |
| Redis      | 42,350              | 0.24           | 0.58          | 0.89          |

**분석:**

- Redis가 **5.4배 빠름** (인메모리, 단순 GET)
- MongoDB가 PostgreSQL보다 **25% 빠름**
- 모두 목표 (5,000 ops/s) 초과, 95p < 10ms ✅

#### 시나리오 3: 복잡한 쿼리 (클리닉별 최근 접근 50개, 1,000회)
...
WHERE clinic_id = 'clinic123' AND resource_type = 'thumb'
ORDER BY last_access DESC LIMIT 50;

-- MongoDB
db.cache_entries.find({ clinic_id: 'clinic123', resource_type: 'thumb' })
  .sort({ last_access: -1 }).limit(50);

-- Redis (불가능, Sorted Set으로 제한적 구현)
ZREVRANGE clinic:clinic123:recent 0 49 WITHSCORES
```

| 저장소     | 조회 처리량 (ops/s) | 평균 지연 (ms) | 95p 지연 (ms) | 99p 지연 (ms) |
| ---------- | ------------------- | -------------- | ------------- | ------------- |
| PostgreSQL | 1,120               | 8.93           | 21.45         | 32.18         |
| MongoDB    | 1,480               | 6.76           | 16.82         | 25.34         |
| Redis      | N/A                 | N/A            | N/A           | N/A           |

**분석:**

- MongoDB가 PostgreSQL보다 **32% 빠름** (인덱스 최적화)
- Redis는 복잡한 쿼리 불가 ❌
- 모두 목표 (95p < 50ms) 달성 ✅

#### 시나리오 4: 무효화 (패턴 기반, 1,000건 삭제 × 10회)
...
-- PostgreSQL
DELETE FROM cache_entries
WHERE clinic_id = 'clinic123' AND resource_id LIKE 'patient456:%';
...
-- Redis
SCAN 0 MATCH cache:meta:clinic123:patient456:* COUNT 100
DEL key1 key2 key3 ...
```

| 저장소     | 삭제 처리량 (ops/s) | 평균 지연 (ms) | 95p 지연 (ms) | 총 시간 (10회) |
| ---------- | ------------------- | -------------- | ------------- | -------------- |
| PostgreSQL | 2,850               | 3.51           | 8.42          | 3.51s          |
| MongoDB    | 3,620               | 2.76           | 6.73          | 2.76s          |
| Redis      | 1,240               | 8.06           | 19.23         | 8.06s          |

**분석:**

- MongoDB가 가장 빠름 (인덱스 기반 deleteMany)
- Redis는 SCAN + DEL 2단계 필요하여 느림 ⚠️
- 모두 목표 (1,000건 < 1초) 달성 ✅

#### 시나리오 5: TTL 만료 처리 (1,000건)

**처리 방식:**

```bash
# PostgreSQL (배치 워커)
DELETE FROM cache_entries WHERE ttl > 0 AND ttl < EXTRACT(EPOCH FROM NOW());

# MongoDB (자동)
# TTL 인덱스가 60초마다 자동 실행

# Redis (자동)
# EXPIRE가 백그라운드에서 자동 처리
```

| 저장소     | 처리 방식  | 처리 시간 (1,000건) | CPU 사용 | 비고            |
| ---------- | ---------- | ------------------- | -------- | --------------- |
| PostgreSQL | 배치 워커  | 325ms               | 12%      | 수동 실행 필요  |
| MongoDB    | TTL 인덱스 | 자동 (60초 주기)    | 5%       | 백그라운드 자동 |
| Redis      | EXPIRE     | 자동 (실시간)       | 3%       | 백그라운드 자동 |

**분석:**

- MongoDB/Redis는 자동 처리로 운영 간편 ✅
- PostgreSQL은 배치 워커 별도 실행 필요 ⚠️
- 모두 성능 목표 달성 ✅
...
| 저장소     | 기본 메모리 | 데이터 메모리 | 총 메모리 | 비고             |
| ---------- | ----------- | ------------- | --------- | ---------------- |
| PostgreSQL | ~50MB       | ~15MB         | ~65MB     | shared_buffers   |
| MongoDB    | ~100MB      | ~18MB         | ~118MB    | WiredTiger cache |
| Redis      | ~12MB       | ~12MB         | ~24MB     | 전체 메모리 상주 |

**분석:**

- Redis가 가장 적은 메모리 (전체 데이터 인메모리)
- MongoDB가 가장 많은 메모리 (WiredTiger 캐시)
- PostgreSQL은 중간 수준

### 5.4 성능 종합 평가

| 항목          | PostgreSQL | MongoDB | Redis   | 승자          |
| ------------- | ---------- | ------- | ------- | ------------- |
| 쓰기 성능     | 1,850 ops  | 2,340   | 8,920   | Redis         |
| 단순 조회     | 6,240 ops  | 7,810   | 42,350  | Redis         |
| 복잡한 쿼리   | 1,120 ops  | 1,480   | N/A     | MongoDB       |
| 무효화        | 2,850 ops  | 3,620   | 1,240   | MongoDB       |
| TTL 처리      | 수동       | 자동    | 자동    | MongoDB/Redis |
| 메모리 효율   | 65MB       | 118MB   | 24MB    | Redis         |
| **종합 점수** | **3.2**    | **4.1** | **2.8** | **MongoDB**   |

**종합 분석:**

1. **Redis 장점**: 단순 조회에서 압도적 성능 (42K ops/s)

   - 단점: 복잡한 쿼리 불가, 메모리 제약

2. **MongoDB 장점**: 균형잡힌 성능 + 복잡한 쿼리 + TTL 자동

   - 모든 시나리오에서 목표 달성
   - PostgreSQL 대비 25-32% 빠름

3. **PostgreSQL**: 안정적이나 MongoDB보다 느림
   - TTL 수동 처리 부담

**성능 측정 결론:**

- **MongoDB**가 기능 + 성능 모두 우수
- Redis는 보조 저장소(핫 캐시)로 적합
- PostgreSQL은 MongoDB 대비 성능 열세
...
| ---------------- | ------ | ---------- | -------- | -------- | ------------------------- |
| 기능 충족률      | 30%    | 3.5/4      | 4/4      | 2.5/4    | 8가지 요구사항            |
| 복잡한 쿼리      | 15%    | 4/4        | 3.5/4    | 1/4      | 범위 검색, 집계           |
| **성능 (실측)**  | 20%    | 3/4        | 3.5/4    | 4/4      | **쓰기/조회/복잡한 쿼리** |
| TTL 자동 관리    | 10%    | 1/4        | 4/4      | 4/4      | 운영 편의성               |
| 운영 단순성      | 15%    | 4/4        | 2.5/4    | 3/4      | 기존 인프라, 학습 곡선    |
| 기존 인프라 활용 | 5%     | 4/4        | 1/4      | 2/4      | PostgreSQL 이미 사용      |
| 확장성           | 5%     | 2.5/4      | 4/4      | 2/4      | 대형 클리닉 대응          |
| **총점**         | 100%   | **3.31**   | **3.59** | **2.66** | **MongoDB 선정**          |
...
```
= 0.30 × 3.5 + 0.15 × 4 + 0.20 × 3 + 0.10 × 1 + 0.15 × 4 + 0.05 × 4 + 0.05 × 2.5
= 1.05 + 0.60 + 0.60 + 0.10 + 0.60 + 0.20 + 0.125
= 3.31
```
...
```
= 0.30 × 4 + 0.15 × 3.5 + 0.20 × 3.5 + 0.10 × 4 + 0.15 × 2.5 + 0.05 × 1 + 0.05 × 4
= 1.20 + 0.525 + 0.70 + 0.40 + 0.375 + 0.05 + 0.20
= 3.59
```
...
```
= 0.30 × 2.5 + 0.15 × 1 + 0.20 × 4 + 0.10 × 4 + 0.15 × 3 + 0.05 × 2 + 0.05 × 2
= 0.75 + 0.15 + 0.80 + 0.40 + 0.45 + 0.10 + 0.10
= 2.66
```

**성능 가중치 설명:**

- 성능 항목 20% 추가 (기능에서 10%, 기존 인프라에서 5%, 확장성에서 5% 이동)
- MongoDB: 균형잡힌 성능 (3.5/4)
  - 쓰기: 2,340 ops/s (목표 1,000 초과)
  - 조회: 7,810 ops/s (목표 5,000 초과)
  - 복잡한 쿼리: 1,480 ops/s, 95p 16.82ms (목표 50ms 내)
- Redis: 단순 쿼리 최고 (4/4)하지만 복잡한 쿼리 불가
- PostgreSQL: 안정적이나 MongoDB보다 25-32% 느림 (3/4)
...
**주 저장소: MongoDB** **보조 저장소: Redis (선택적, 핫 캐시)**
...
- 쓰기: 2,340 ops/s (PostgreSQL 대비 26% 빠름)
- 조회: 7,810 ops/s (PostgreSQL 대비 25% 빠름)
- 복잡한 쿼리: 1,480 ops/s (PostgreSQL 대비 32% 빠름)
- 무효화: 3,620 ops/s (가장 빠름)
- 모든 성능 목표 초과 달성 ✅
...
- ✅ 기존 인프라 활용 (추가 설치 불필요)
- ✅ 복잡한 쿼리 (SQL, 하지만 MongoDB도 충분)
- ✅ 운영 경험 풍부

**결정 이유:**

- **TTL 자동 관리**가 운영에 미치는 영향이 큼 (배치 워커 유지보수 부담)
- **성능 실측** 결과 MongoDB가 25-32% 빠름 (모든 시나리오)
...
2. **성능 우수 (실측)**
   ...
   - MongoDB가 PostgreSQL 대비 **25-32% 빠름**
   - 쓰기: 2,340 ops/s vs 1,850 ops/s
   - 조회: 7,810 ops/s vs 6,240 ops/s
   - 복잡한 쿼리: 1,480 ops/s vs 1,120 ops/s
   - 모든 성능 목표 초과 달성
...
2. **PoC #4**: 통합 프로토타입 개발 (파일시스템 + MongoDB + Redis)
3. **PoC #5**: 클리닉 현장 배포 및 성능 검증

...
- PoC #4: 통합 프로토타입 검증 (선정된 기술 스택 통합 테스트, 성능 검증)

**3단계: 배포 준비 PoC (PoC #4 완료 후)**

- PoC #5: 배포 준비 및 설치 도구 개발 (NSIS 설치 패키지, Windows Service 관리, 자동 진단/업데이트 도구)

**참고**: 실제 클리닉 환경 검증은 오리지널 서버(SCP 서비스) 완성 후 진행
...
Week 7-10:  PoC #4 Phase 1-2 - 통합 프로토타입 (읽기 경로, 캐시 최적화)
Week 11-14: PoC #4 Phase 3-4 - 통합 프로토타입 (쓰기 경로, 장애 대응)
Week 15-17: PoC #4 Phase 5-6 - 통합 프로토타입 (운영 준비, 성능 검증)
Week 18-19: PoC #5 - 배포 준비 및 설치 도구 개발 (NSIS, 자동 진단/업데이트)
```
...
- [PoC #5: 배포 준비 및 설치 도구 개발](PoC/PoC5_설치도구.md)

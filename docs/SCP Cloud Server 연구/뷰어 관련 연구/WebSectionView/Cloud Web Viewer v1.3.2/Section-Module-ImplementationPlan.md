...
- [x] **완료(2026-07-16 재정의) — 우리 구현 = 호장(arcMm) 1D 앵커(§4).** "3D 좌표"는 렌더링이 아니라 **앵커링**을 뜻함 — 2D 단면이지만 3D 볼륨을 곡선 따라 자른 것이라, 계측을 화면 픽셀이 아닌 **기하(호장)에 앵커링**해 slice 스크롤·Interval 변경에도 살아남게 한다. **UI 정본 = 호장 앵커**: `SectionMeasureOverlay` `arcMm=(windowStart+tile)*interval`, 스크롤/interval 변경 시 `round(arc/interval)` 단면 재표시·9-window 밖 숨김(데이터 유지, `isOverlayVisibleOnArc`). **MMI 일반형(평면 point+normal, 거리≤±Interval/2 AND normal≤5°)** 은 core `overlay/overlayPlane.ts`에 구현·테스트(UT-OVL-001/002/003 12케이스, D3=`OVERLAY_NORMAL_TOLERANCE_DEG` 5°)해 뒀으나 **단일 곡선 Section엔 과하여 UI 미배선** — 다중 Curve/MPR 공유 시 승격(그때 curve tangent를 UI에 전달). **잔여(선택)**: MT-OVL-004(스크롤 재표시 시각 확인, 기획팀 데모 테스트에서 함께).
...
- [x] **구현 완료(2026-07-16).** Initialize All(MMI 미정의→§3.10 정의) = **"데이터·값" 초기화 + "모드·토글" 유지**: Curve 제거(→파노라마/섹션 blank)·계측 제거·**Pan/Zoom 값→identity**·WL=볼륨 기본·섹션/파노라마 파라미터 기본; **활성 도구·Show/Hide Overlay·Grid 토글은 유지**(App이 toolStore 미변경). 결과=CT 최초 로드처럼 Scout만. 재오픈 복원(저장본)과 별개. `SectionViewer` 핸들 `initializeAll()`=`applyProjectState(freshDefaultState)`(emptyState + 볼륨 WL·중앙 slice, views는 identity). `toolStore`에 `initializeAll`/`resetCloudWork`(tick), `CwToolbar` commandHandler 배선, App effect가 tick 처리(Initialize All→`sectionViewerRef.initializeAll()`+`deactivateInteraction()`; **Reset Cloud Work→"접목 시 지원" 안내 다이얼로그만·상태 불변**). 파노라마·섹션은 default Curve로 재계산(없으면 blank). 아래 계획대로 구현: Initialize All = **default project file 상태로 복귀**(§3.10·§12-D30, CW `CTViewerControllerCore.initializeAll`→`reloadDefaultProjectFile` 정본). **초기화 대상 = Save 항목(§7 ①~⑫)의 역연산**이라 Save 데이터 모델(`SectionProjectState`, T-P5-1·4)이 확정된 뒤에 구현해야 대상·기준이 일치한다(그래서 P5 말미에 배치). **구현:** `CwToolbar commandHandler`에 `initializeAll` 배선 → 전 뷰의 계측/주석·Pointer 그림 제거 + WL/필터/Inverse 기본값 + Pan/Zoom(=Reset View) + Curve·섹션 파라미터(두께/간격/폭/높이·B/L·center) default + neutral 도구; 파노라마·섹션은 default Curve로 재계산(default Curve 없으면 blank). 본 모듈 "default" = **콘텐츠 오픈 시 로드된 초기 상태**(저장 prj 있으면 그 값, 없으면 blank) — 로드 시 스냅샷을 보관해 그 값으로 복원. **`resetCloudWork`는 구현하지 않는다(구현 불가)** — `resetMetaData`=`ioApis.resetMetaData`(CW 호스트 주입 클라우드 서버 IO)·`reloadState.requestReload`(CW 워크스페이스 리로드) 의존, standalone 모듈에 없음. **버튼 클릭 시 "추후 CW 접목 시 지원됩니다" 안내(toast/알림)만** 표시하고 아무 상태도 바꾸지 않는다(로컬 initializeAll로 대체하지 않음 — 서버 저장분 폐기·리로드라 의미가 다름). 실제 동작은 **접목 시 CW 제공**(§9.10). `requiresEditControl` 게이팅은 CW 셸 책임(데모 생략 가능).

...
To. 임지수/ Jessi, 김성훈 / Scott , 민진우/ Thomas (cc. 심명희/ Liz , 조상형(Leo) , 박서연/ Ruby , 김성호_ES/ Owen, 탁수용/ Nick )

https://vks.vatech.com/spaces/ESDEVELOPER/pages/320005969/Cloud+Web+Viewer+v1.3.2+%E2%80%94+Section+Module+%EA%B0%9C%EB%B0%9C+OnePager

안녕하십니까.

위 링크에 Section 모듈 스펙 문서(OnePager)를 작성하여 리뷰 요청드립니다.

본 문서는 기존 MMI와 PLAN-1287 기획 요구사항을 정리한 것입니다. MMI 1.1~1.14 전 기능을 매핑하였고, B/L 자동 판정은 이번에 회신 주신 규칙(P1→P2 선분 기준으로 CT 단면 중심 C가 있는 쪽 = L, 반대쪽 = B)을 반영하였습니다. MMI에 이미 상세히 정의되어 있고 구조적으로 큰 문제가 없다고 판단하여, 오늘부터 바로 구현을 시작하려 합니다. MMI·기획 요구사항과 다른 점이 있으면 말씀 부탁드립니다.

본 스펙에서 중요한 두 가지는 다음과 같습니다.

1. 기존 CloudWebViewer와 개발 환경(Node/pnpm·React·Vite·MUI·zustand, 툴바 store, prj 스키마 등)을 일치시켜 접목을 최대한 쉽게 하는 것입니다.
2. Section 모듈의 구현 범위를 정의하고, CloudWebViewer에 어떻게 접목되는지 설명하였습니다. Section 뷰는 개발실에서 WebGL로 구현하여 패키지로 제공하고, CloudWebViewer가 이를 embed하는 방식입니다.

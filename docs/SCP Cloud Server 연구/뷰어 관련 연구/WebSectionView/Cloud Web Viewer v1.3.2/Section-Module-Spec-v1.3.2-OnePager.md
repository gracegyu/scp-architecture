...
| **커서 `CW_CURSORS`**(Pan·Zoom·Pointer·Disable·Erase·Length·Angle·FreeDraw·**Move**·**Arrow**) | `packages/components/src/cursors.ts` | 모듈 복사 + **Arrow 자체 제작**(§3.7·§3.8·§3.4.2·§3.9·§11) | CW `workSpace/setting` `CURSORS` + `vtkjs-wrapper` `overlaySelectedCursor`(Move). **Arrow는 CW에 없어 개발실 제작** | **CW import로 교체·복사본 삭제**. Arrow 커서는 개발실 제작본 유지(접목 시 CW 정식본 있으면 교체 가능) |
...
- **Arrow 전용 커서 — 해결(2026-07-16, 개발실 자체 제작).** 계측/주석 커서는 CW `CURSORS` 정본을 그대로 쓰나(length→LENGTH·angle→ANGLE·freeDraw→FREEDRAW·Pointer Pen→POINTER·Eraser→ERASE), **Arrow는 v1.3.2 신규라 CW에 전용 커서가 없었다.** 기획 제작을 기다리지 않고 **개발실이 `cursors.ts`에 `ARROW` 커서를 직접 제작**(계측군 정합: LENGTH/ANGLE와 동일 좌상단 포인터+화살표 글리프, 32×32 흰 채움·검정 외곽 SVG, 핫스팟 3,3). `SectionMeasureOverlay` arrow→ARROW 배선. 기획 요청 불필요. (접목 시 CW가 정식 Arrow 커서를 만들면 그때 교체 가능하나 필수 아님.)
- **계측 크로스뷰 연속 추적 — 해결(2026-07-16).** 커서가 캔버스(뷰)를 벗어나도 **window `mousemove`(+드래그는 `mouseup`)** 가 이어받아 뷰 안에서 끊김 없이 진행된다. 두 경로 모두 커버: **① 드래그**(편집 이동/핸들·freeDraw) = `dragActive`로 window 부착, `moveDrag`/`finishDrag` 캔버스·window 공용(refs 가드로 중복 mouseup에도 멱등); **② 2클릭 미리보기**(length/angle/arrow) = 클릭 사이 rubber-band 미리보기도 `previewing`(draft 진행 중)일 때 window `mousemove`로 tile clamp 추적(캔버스 안에서 얼어붙던 문제 해소, freeDraw와 일관). `localFromClient`로 캔버스 밖 client 좌표도 로컬 변환. **경계에서 멈춤(clamp)은 사양**(계측은 각 section별·경계 넘나들 불가, MMI 1.13·§4) — 이번 수정은 "얼어붙음" 제거일 뿐 경계 clamp는 유지.
...
| 버전 | 일자 | 변경 |
|------|------|------|
| 1.69 | 2026-07-16 | **계측 크로스뷰 연속 추적 해결(§11)**: 커서가 캔버스를 벗어나도 window 리스너가 이어받아 뷰 내 연속 진행. **①드래그**(편집 이동/핸들·freeDraw) = `dragActive`+`moveDrag`/`finishDrag`(캔버스·window 공용·멱등)+`localFromClient`. **②2클릭 미리보기**(length/angle/arrow) = `previewing`일 때 window `mousemove`로 미리보기 tile clamp 추적(캔버스 안에서 얼어붙던 문제 해소, freeDraw와 일관). 경계 clamp는 사양 유지(§4). 빌드 5/5·테스트 통과. |
| 1.68 | 2026-07-16 | **Arrow 전용 커서 자체 제작(§11 해결·§3.4.2)**: CW에 없는 Arrow 커서를 기획 대기 없이 **개발실이 직접 제작** — 계측군 정합(LENGTH/ANGLE와 동일 좌상단 포인터 핫스팟 3,3 + 화살표 글리프, 32×32 흰 채움·검정 외곽 SVG base64) `cursors.ts` `ARROW` 추가, `SectionMeasureOverlay` arrow→ARROW 배선(임시 FREEDRAW 대체). §11 숙제 해소. |
| 1.67 | 2026-07-16 | **죽은 코드 정리 + S/P/S 비율 locale 무관 고정**: devChildren(개발용 컨트롤) 제거로 남았던 미사용 심볼 정리(PanoramaView 스타일 상수·`handleGenerate`·`lastMs`·`setPanoramaProjectionPreset`, SectionGrid 렌더/연산 모드 props·`sectionComputeModeShortLabel` 등 → components 미사용 0). **레이아웃 버그 수정**: SectionViewer 그리드를 `minmax(0, 1fr/2fr)`로 바꿔 **뷰 라벨 길이(Scout↔스카우트 등 locale 차이)가 S/P/S 3분할 비율을 바꾸지 않도록** 고정(기존 `1fr` min-content가 트랙을 넓혔음). |

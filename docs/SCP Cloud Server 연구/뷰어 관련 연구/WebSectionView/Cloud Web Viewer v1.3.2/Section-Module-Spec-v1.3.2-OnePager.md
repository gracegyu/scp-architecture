...
| **Initialize All 초기화 기준** | MMI 1.13-1a는 버튼만 나열·기준 미정의 | **"default project file 상태"로 복귀**(CW `CTViewerControllerCore.initializeAll`→`reloadDefaultProjectFile`). 파노라마·섹션은 Curve 파생물이라 "생성 전/후"가 기준이 아님 — Curve를 default로 되돌리면 재계산(default Curve 없으면 blank). 본 모듈 "default"=콘텐츠 오픈 시 로드된 초기 상태(저장 prj 있으면 그 값, 없으면 blank). 대상=Save §7 ①~⑫ 역연산. 개발실 결정(2026-07-16, §3.10·§12-D30) |
...
- **Initialize All = 우리도 구현한다(클라이언트 동작).** default로 되돌리기는 우리 상태만 리셋하면 되므로 구현 가능. 본 모듈에서의 "default project file"은 **콘텐츠 오픈 시 로드된 초기 상태**(저장 prj가 있으면 그 Curve/설정, 없으면 blank)로 정의한다(개발실 결정 §3.4.2·§12-D30). Initialize All = 그 초기 상태로 되돌리기 = **주석/계측·Pointer 그림 전부 제거 + WL/필터/Inverse 기본값 + Pan/Zoom(Reset View) + Curve·섹션 파라미터(두께/간격/폭/높이·B/L·center) default + neutral 도구**, 파노라마·섹션은 default Curve로 재계산(또는 blank). 이는 **Save 항목(§7 ①~⑫)의 역연산**이다. (IP T-P5-5)
...
**현재 상태(POC)·구현 계획:** 툴바에 두 버튼은 표시되나 현재 **핸들러 미배선(no-op)** — `CwToolbar` `commandHandler`는 `resetView`만 연결(§3.7). **Initialize All(로컬 default 복귀)은 구현 예정 = IP T-P5-5**, Save Project(§7·T-P5-*)의 역연산이라 **Save 데이터 모델 확정 후(P5 말미)에 착수**. **Reset Cloud Work는 미구현 확정 → 클릭 시 "접목 시 지원" 안내**(T-P5-5), 실제 동작은 **접목 시 CW 소유**(§9.10).

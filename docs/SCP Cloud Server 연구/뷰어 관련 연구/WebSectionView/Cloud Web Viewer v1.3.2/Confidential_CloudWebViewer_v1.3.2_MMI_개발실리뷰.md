...
- BL/LB 기준점 이동: 첫 입력 point에 생성되는 기준점을 곡선 위에서 이동해 B/L 표기 기준을 바꾼다(PoC 없음, 개발실 확정 대상).
  - 구현 가능성 조건부 · 구현 공수 중 · 추가 PoC 조건부. B/L 결정 로직 확정에 종속된다.
...
| 1.5 | Section view(이미지) 표시 시점 | Curve 입력 완료 시 표시, 완료 전까지 blank(MMI 289줄) | 상이(표시 시점 차이) | **권고: MMI 방식(완료 후 1회 생성·표시) 채택.**<br>드로잉 중 이미지 재생성이 없어 성능상 안전하고 추가 부담 없음.<br>(연속 재생성의 핵심 리스크는 이 항목이 아니라 4.2 Section Slice 변경) |
| 1.7 | Section 폭 조절 | Center line control point 드래그 | Slider | 조작 방식 통일(6장 모바일 정책과 연계) |
| 1.8 | Section 세로 폭(Height) 조절 | 경계선 드래그 | Slider | 조작 방식 통일 |

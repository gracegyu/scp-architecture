...
| 항목 | 결정 |
| --- | --- |
| **연산 위치** | 우선 **클라이언트 단일 스레드 JavaScript** 로 trilinear 샘플 + 루프 구현. **생성 시간(ms) 및 곡선 변경 시 재계산 시간** 을 로그/UI로 기록한다. |
...
| 3 | Panorama View에 결과 바인딩, WC/WW 적용 | UI 통합 |
...
2. 파노라마 생성 Core + PanoramaView 연동

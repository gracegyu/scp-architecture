...
- **기획/제출(초안)**: 2026-05-05
- **문서 갱신**: 2026-05-06 (scp-section-poc 구현과 동기화)
- **상태**: PoC 구현 진행 — Core(`packages/core` `panorama` 모듈) 및 `PanoramaView` 연동 반영됨. 본 문서는 설계·알고리즘과 **현재 동작**을 함께 둔다.
...
Phase 3에서 Scout Axial 상에 정의한 **치열궁 곡선(녹색 스플라인)** 과, 곡선에 **수직인 샘플 방향(빨간 세그먼트)** 을 사용하여, 메모리에 적재된 **3D CT Volume**에서 **파노라마용 2D 이미지**를 생성한다. 데모에서는 Volume이 있으면 Panorama 영역에 **Canvas 2D**로 비트맵을 스케일 표시하고, Volume이 없으면 Phase 1과 같이 **WebGL 정적 텍스처** 경로를 유지한다(메인 OnePager 「뷰별 렌더링 경로」 참조).
...
### 4. 슬랩 두께 방향의 값 결합 (투영 방식 선택)

빨간 선 방향(법선 `N̂`)으로 여러 샘플을 얻었을 때, 슬랩 내 HU를 **한 스칼라**로 줄인다. **Panorama UI에서 사용자가 방식을 선택**하고 `파노라마 생성` 시 적용한다.

| 방법 | 설명 | PoC |
| --- | --- | --- |
| **MIP** | 슬랩 내 **최댓값** | 고밀도(금속·겉뼈) 강조. 상용에서도 흔히 쓰이나 **전체가 밝고 밋밋**해 보일 수 있음. |
| **Mean** | 슬랩 내 **산술 평균** | 얇은 슬랩과 함께 쓰면 **중간 명암**이 상대적으로 살아나 상용 파노라마에 가까워 보일 때가 있음(설정에 따라 뿌옇게 될 수도 있음). |
| **Percentile (50~95%)** | 슬랩 HU 정렬 후 **선형 보간 백분위** | 순수 MIP보다 극단값(금속 한 방) 영향이 줄어 **대비**를 맞추는 튜닝용으로 비교 실험에 적합. UI에서는 **50·60·70·80·90·95%** 프리셋을 고른 뒤 생성에 반영한다(50%는 중앙값에 가깝게 해석). |

**본 PoC**: 위 세 계열을 **모두 구현**하고, 기본 선택은 **MIP**. 임상·CleverOne 유사 화질은 **슬랩 두께·WC/WW·투영 방식**을 같이 맞추는 과제로 남긴다.

**Scout 빨간 수직선** 길이(Phase 3 오버레이)와 **실제 파노라마 슬랩 반폭(`slabHalfWidthMm`)** 은 코드상 별개일 수 있으니, 비교 실험 시 혼동하지 않는다.
...
| **표시** | Volume 모드: `ImageData` + 보이지 않는 소스 캔버스에 `putImageData` 후 표시 캔버스에 `drawImage` 로 스케일. Volume 없음: 기존 **단일 WebGL 텍스처 쿼드**. |
...
| 방식 | 장점 | PoC에서의 문제 |
| --- | --- | --- |
| **곡선 편집마다 자동** | 상용 뷰어에 가까운 흐름 | 제어점 드래그 등으로 이벤트가 연속 발생하면 **메인 스레드 샘플링 루프**가 겹쳐 UI가 끊길 수 있음. “한 번 생성”에 걸린 시간과 **부분 갱신·중복 호출**이 섞여 벤치마크가 애매해짐. |
| **명시적 생성 버튼** | 호출 시점이 분명해 **ms 측정**과 성공 기준(목표 시간) 정의가 쉬움. 편집 중에는 Scout만 갱신하면 됨 | 곡선을 고친 뒤 **버튼을 한 번 눌러야** 결과가 바뀜 |

**본 PoC 채택(곡선·슬라이스·투영)**: **명시적 “파노라마 생성” 버튼**으로 전체 파노라마를 재계산한다. **곡선 변경·Axial 슬라이스 변경·투영(MIP/Mean/백분위) 프리셋 변경만으로는 자동 재생성하지 않는다**; 결과를 보려면 버튼을 누른다.

**추가 채택(WC/WW)**: `SectionViewer`의 **`useScoutAxialUi`** 로 Scout·Panorama가 **동일한 Window Center / Window Width** 를 공유한다. **이미 최소 한 번 파노라마 생성에 성공한 뒤**에는, Scout 또는 Panorama 툴바의 WC/WW 슬라이더를 바꾸면 **약 280ms 디바운스** 후 **자동으로 동일 파이프라인으로 재생성**된다(슬라이더 연속 드래그 시 마지막 값 기준). 아직 파노라마가 없을 때 WC/WW만 바꾸는 경우에는 자동 생성하지 않는다. 새 Volume(CT) 로드 시 파노라마 비트맵은 초기화되고, 위 “한 번 생성 성공” 플래그도 리셋된다.
...
- Volume 미로드 또는 곡선이 최소 조건(제어점 **2개 이상**)을 만족하지 않으면 생성 버튼 **비활성**, 메타 영역에 짧은 안내 문구.
- **슬랩 투영**: Panorama 툴바 `<select>` 에서 **MIP / Mean / 백분위 50·60·70·80·90·95%** 중 선택 후 **파노라마 생성**으로 반영. 투영 프리셋만 바꾼 경우에도 이미지는 바뀌지 않으므로 **재생성 버튼**이 필요하다.
- **WC/WW**: 생성 시 Core에 전달되어 `ImageData` 로 **윈도잉이 적용**된다. Scout 하단과 동일 값이며, Panorama 툴바에도 **Pan WC / Pan WW** 슬라이더가 있어 같은 상태를 조절할 수 있다. **이미 파노라마가 있으면** WC/WW 변경 후 **디바운스 자동 재생성**(위 절 참조).
- **마지막 생성 시간**: 툴바에 **마지막 소요 ms** 를 표시한다.
- 생성 중에는 생성 버튼·투영 선택·WC/WW 슬라이더를 **비활성**하여 **중복 실행**을 막는다.
- Volume 없을 때는 Phase 1과 같이 정적 `panorama.png` 를 WebGL 쿼드로 표시한다. Volume+`axialUi` 가 있으면 **Canvas 2D** 에 생성 비트맵을 스케일하여 그린다.
- 구현·결과 문서에는 **버튼·투영·WC/WW 위치**(Panorama 상단 툴바)를 Phase 3 레이아웃 설명과 같이 남긴다.
...
| MIP만으로 임상 파노라마와 톤이 다름 | 중간 | 중간 | PoC는 **Mean·다단계 백분위(50~95%) 투영**과 **WC/WW·디바운스 재생성**으로 비교 실험. 슬랩·제품 스펙 대비는 별도 |
...
| 2 | 호장 샘플링 + trilinear + 슬랩 **MIP·Mean·Percentile** 루프 | Core 함수 |
| 3 | Panorama 상단 **투영**(MIP/Mean/백분위 50~95%) + **파노라마 생성** + **Pan WC/WW**(공유 상태)·**WC/WW 디바운스 자동 재생성**·**마지막 ms** | UI 통합 |
...
1. Phase 3 곡선을 바꾼 뒤 **파노라마 생성 버튼**으로 **재생성**할 수 있다(곡선·슬라이스·투영에 대한 **연속 자동 재생성**은 채택하지 않음).
2. 생성된 파노라마에서 **치열이 펼쳐져** 시각적으로 판독 가능하다(정량 SSIM 등은 PoC 필수 아님).
3. **Chrome 기준** 생성 시간이 메인 OnePager 목표(**1초 이내**)에 근접하거나, 초과 시 원인·완화책을 결과에 기록한다. UI에 **마지막 생성 ms** 가 표시되면 측정·스크린샷에 유리하다.
4. 투영 방식은 **UI에서 MIP·Mean·백분위(50·60·70·80·90·95%) 중 선택** 가능하며, 문서·코드에 옵션과 기본값(MIP)을 명시한다.
5. **WC/WW** 는 생성 파이프라인에 반영되며, **최초 생성 성공 이후** WC/WW 변경 시 **디바운스 자동 재생성**으로 윈도 조절을 반복 실험하기 쉽게 한다.

### Phase 4 산출물

1. 본 OnePager 및 Phase 4 결과 md(구현 후)
2. 파노라마 생성 Core(`generatePanoramaImageData`, `DEFAULT_PANORAMA_OPTIONS` 등) + `PanoramaView` 연동: **투영(MIP/Mean/백분위 50~95%)**, **파노라마 생성** 버튼, **Pan WC/WW** 슬라이더, **마지막 ms** 표시, **WC/WW 디바운스 자동 재생성**, Volume 교체 시 비트맵 초기화
3. `SectionViewer`에서 `useScoutAxialUi` 로 Scout·Panorama **슬라이스·곡선·WC/WW 공유**
4. 본 문서 도해와 동일 의미의 **스크린샷**(곡선·파노라마 나란히) — 검수용

### 코드 위치 (참고, scp-section-poc)

| 영역 | 경로 |
| --- | --- |
| 알고리즘·옵션 | `packages/core/src/panorama/panorama.ts` |
| UI | `packages/components/src/PanoramaView.tsx` |
| 공유 WC/WW·곡선 | `packages/components/src/hooks/useScoutAxialUi.ts` |

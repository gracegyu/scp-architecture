...
- 공유 사항
  ...
  - **S2. 버그 리포트 (ESCV-138 Sub-Task) — 프레임 유지·추가 접수분 반영** — 7/23까지 접수 **25건(ESCV-144~169) 전부 수정 완료**(상세는 **7/23 스냅샷** 참조). 아래엔 **검증 대기분 + 이번 주 신규 접수분**만 유지·추가. 확인 후 **Resolved→Close는 기획팀**.
    ...
    | [ESCV-175](https://vts.vatech.com/browse/ESCV-175) | 개선 | Scout·Panorama 오버레이 | 확대 시 Curve·section line·텍스트가 이미지처럼 같이 굵어지고 계단(래스터) 발생 → 오버레이를 **device 해상도로 재렌더**, 위치·구간선 길이만 이미지와 함께 스케일하고 **선 굵기·글자·핸들은 확대해도 화면상 일정(얇게)·또렷**(계측 오버레이와 동일 패턴). 두 뷰 모두 반영 | ✅ **수정 완료**(시각검증 권장) |
    | [ESCV-176](https://vts.vatech.com/browse/ESCV-176) | 개선 | Section B/L 방향 | curve 그리는 방향에 따라 Section 좌우 B/L이 반전 → **협측(B)=치열 중심에서 먼 쪽=항상 좌측**으로 기하 고정(Clever One/Ez3D-i 정합). Section 샘플링 법선 정렬(JS+WASM 래퍼, .wasm 재빌드 불필요)·자동 극성 'normal' 고정·Scout B/L 라벨 기하정렬. ESCV-150(기준점 반전)·수동 L/B Switch는 상대 반전으로 유지 | ✅ **수정 완료**(재오픈 1회: 이미지가 반대로 flip → Section 이미지 부호 정정(협측=좌), Scout 라벨은 바깥=B로 원복. **Section·Scout 협/설 정합 최종 확인**. Scout·Section이 C 기준 부호 반대인 건 Section 이미지 내부 미러 때문·정상) |
    | [ESCV-155](https://vts.vatech.com/browse/ESCV-155) (재오픈) | 버그 | Draw Curve 미리보기 | 첫 점(P1)+커서부터 **Panorama thickness·navigator·section line 다발·호장 길이 라벨**이 커서 따라 실시간 미리보기 안 됨(완료 후에만 표시) → 곡선 파생 오버레이의 arc 컨텍스트를 **미리보기 곡선([controlPoints, cursor])** 기준으로 전환, 2점 미만(1점+커서)에서도 표시. 완료/비-Draw 렌더는 불변 | ✅ **수정 완료**(시각검증 권장) |
    | [ESCV-157](https://vts.vatech.com/browse/ESCV-157) (재오픈) | 버그 | Draw Curve 미리보기 | 1개 점(P1) 상태부터 **section line·active section line**이 미리보기되어야 함 → **ESCV-155 수정으로 동시 해결**(미리보기 곡선 기준 + `effCenterMm`). 별도 변경 없음. 155와 함께 close 예정 | ✅ **수정 완료**(155 동시 해결) |
    | [ESCV-161](https://vts.vatech.com/browse/ESCV-161) (재오픈) | 버그 | 휠 slice(Draw/Edit) | Draw/Edit Curve 모드에서 휠 slice 이동 미동작(slider만) → MMI Appendix 1대로 두 모드도 ● 지원. Scout·Panorama 휠 핸들러의 `drawMode/editMode` early-return 가드 제거 | ✅ **수정 완료** |
    | [ESCV-177](https://vts.vatech.com/browse/ESCV-177) | 개선/스코프 | prj 초기 표시(다중 커브) | Clever One(.e3prj)이 커브 2+개 저장 시 표시 기준 → **Jessi 확정: v1.3.2 웹은 단일 표시, ActiveCurveID 커브 기준**(MMI 1.13 §6.1 단일 커브 정합). 포맷은 CO·CW 동일 `.e3prj`(스펙+샘플 보유), Desktop→Web 단방향. `cwPrjAdapter`를 ActiveCurveID 선택으로 반영. 접목 시 형제 커브 보존 규칙 OnePager §D32에 정리(CW팀 부재). **Jessi 제공 샘플 검증: 커브 7개·ActiveCurveID=Curve_000005(첫 번째 아님)로 선택 로직 필요성 확인. 실측 발견 — ControlPointList가 `\|`-구분 3D 환자-mm(어댑터 2D는 placeholder) → 접목 시 3D↔2D 변환 필요(§D5)** | ✅ **어댑터·좌표변환·prj 계약·dog-food 완료**(A1~4): core 좌표변환기·`\|`-3D mm 어댑터·형제커브 보존 병합·`SectionContentHandler` 계약(§9.5.1)·데모 localStorage 3D mm 왕복·스키마 v2. 실제 샘플로 Curve_000005 선택·3D 파싱 검증(core 119 UT). 접목 시 mm↔픽셀은 실볼륨 geometry·완료 후 Jira 확인 |
  ...
  - **S3. 일정 — 버그 수정·검증 마무리 + 접목 착수(Sonya).**
    ...
    gantt
        ...
        인계(핸드오프)                       :milestone, ho, 2026-07-30, 0d

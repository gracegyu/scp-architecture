...
| **운영 비용 전가** | 토큰 갱신, 자격증명 관리, 서버 가용성, 장애 대응 등 운영 부담이 모두 Vatech으로 이전 |
...
| 항목 | 불리한 점 |
|------|----------|
| **비대칭 투자** | Straumann은 기존 API 제공만 하면 되고, Vatech이 Auth Server + 연동 로직 전체를 개발/운영. 비용 부담이 일방적 |
...
- Auth Server 구축 비용과 운영 책임을 **100% Vatech에 전가**하면서 비용 분담이나 기술 지원에 대한 언급 없음
- 기존 계획에 없던 인프라를 추가로 요구하면서 **구체적인 지원 방안이 없음**
...
| | 비용 분담 | **거부 가능성 높음** |
| **Vatech** | API Gateway 인프라 구축/운영 | AWS 서버리스로 부담 최소화 가능 |
| | 연동 로직 개발 (Lambda 함수) | Straumann 스펙 기반으로 개발 |
| | 운영 비용 (AWS 종량제) | 트래픽 적으면 저렴 |

Straumann 입장에서 Vatech이 자비로 API Gateway를 구축해주겠다는 것이므로 **반대할 이유가 전혀 없다**. 비용 분담을 거부하더라도 Vatech이 AWS 기반으로 독립적으로 구축을 진행하는 데 기술적 장애물은 없다.
...
| 단계 | 내용 | 주체 | 선행 조건 |
|------|------|------|----------|
| **1. 구축 의사 전달** | Vatech → Straumann: "API Gateway 방식으로 구축하겠다. 위 1~6번 항목을 제공해달라" | 호주 법인/ES | 내부 의사결정 완료 |
| **2. 스펙/환경 수령** | Straumann → Vatech: Portal 접근, OAuth 스펙, API 명세, 샌드박스, 자격증명 제공 | Straumann | 1단계 완료 |
| **3. AWS 인프라 구성** | API Gateway, Lambda, Secrets Manager, DynamoDB, NAT Gateway 프로비저닝 | ES | 2단계와 병행 가능 |
| **4. Lambda 개발** | 토큰 관리, 환자 동기화, 문서 업로드 등 Lambda 함수 개발 | ES | 2단계 완료 필수 |
| **5. 테스트** | 샌드박스 환경에서 E2E 검증 | ES | 3~4단계 완료 |
| **6. 운영 배포** | 프로덕션 전환 | ES | 5단계 완료 |

> **참고**: 4/30 회의록 Action Item에서 호주 법인이 5/15까지 Straumann과 재논의 후 ES에 공유하기로 했으나, 현재 공유 여부 미확인. 1단계 진행 전에 호주 법인 담당자에게 진행 상황 확인 필요.
...
| 3 | **비즈니스 우선순위 재평가** | 현재 호주 시장 장비 판매 지원 목적이므로, 구축 비용 대비 실질 매출 효과를 정량적으로 검토 후 진행 여부 결정 |

...
- 논의 사항 (7/2 신규)
    ...
    - **R4. 프록시(B/C) 에러·타임아웃 정책 (결정 필요)** — GW 중계 시 서버 연결 실패·네트워크 일시 장애·응답 지연 처리가 인터페이스에 정의돼 있지 않던 공백을 채움. **정책 골격(아래)은 SRS §7.5.4·§7.7.4·OpenAPI에 이미 반영**했고, 회의에서는 **수치·서킷 v1.0 범위만 결정**하면 됨(Appendix B #25).
        - **정책 골격(반영 완료 · 확인용)**:
            ...
            - 오류 매핑: GW 생성(연결실패 502 / 타임아웃 504 / 서킷 503)=GW envelope; **upstream 자체 4xx/5xx=verbatim 통과**. `Vatech-Error-Origin: gateway|upstream` 마커로 책임 구분.
        - **결정 항목 (설명 · 추천안 — 승인/조정 요청)**:
            ...
            | D9 | 오류 매핑·`Vatech-Error-Origin` | GW 장애 vs upstream 거부를 호출자가 구분(장애 추적) | **확정**(골격대로) | SRS §7.7.4 표 반영 완료 |

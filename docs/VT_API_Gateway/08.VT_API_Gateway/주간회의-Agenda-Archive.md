> **아카이브** — 6/25~8/6 주간회의 Agenda 과거 스냅샷 보존(가끔 조회용). 현행 주차는 `주간회의-Agenda.md`.
  ...
  - EzServer와 Vatech API Gateway targets 연동 계획 (Thomas)
    - 추가 대상
      ...
      - (?)Dentbird 연동: ~10/30
        ...
        - PLAN-1270 - 이마고웍스사와 Dentbird 서비스 연동을 위한 협업을 검토한다. **In Progress** [](https://vts.vatech.com/secure/viewavatar?size=xsmall&avatarId=12218&avatarType=issuetype)
    - 이슈
      ...
      - Dentbird와 같은 연동은 동시에 1개가 아니라 다중이 될 수도 있을 거 같음
        ...
        - EzServer에 target 정보와 region 정보를 VAG로부터 동적으로 받아서 설정가능하도록 할 필요가 있음 - 사용자는 target과 region을 설정하면 EzServer는 VAG로부터 endpoint를 받도록 한다.
    - Raymond 답변
      - 추가 대상(AXS/CleverSpace/CleverRC·CleverDent·Weclever/Dentbird)
        - → GW: 신규 target은 GW 레지스트리 1행 추가로 수용합니다(코드·경로 변경 없음). 이 부분은 GW SRS 수정이 필요 없습니다.
      - 이슈 — EzServer 중개 전용 연동(Dentbird)·EzServer 업그레이드 필요
        - → GW: 경계 확인이 필요합니다. Dentbird가 (a) GW를 통과하는 target인지, (b) EzServer 자체 reverse-proxy(GW 비경유)인지에 따라 다릅니다. (b)면 GW 범위 밖(EzServer 소유)이고, (a)면 기존 target-routed proxy로 그대로 수용됩니다.
      - 이슈 — 다중 동시 연동(CleverSpace + Dentbird)
        - → GW: 클리닉당 다중 target은 GW 레지스트리와 org-mapping으로 이미 수용됩니다. GW SRS 수정이 필요 없습니다.
      - 이슈 — EzServer가 target·region을 VAG로부터 동적 수신
        - → GW: 리전은 이미 Region Directory로 제공되고(country→region), target 호스트는 `{target}.<region>.gw.<루트도메인>` 결정형이라 EzServer는 별도 endpoint 조회 없이 서브도메인 변환만 하면 됩니다. target 목록을 GW가 API로 내려주는 discovery가 필요하다면 신규 GW 기능으로 별도 스코핑하겠습니다. 회의에서 필요 여부를 정해 주세요.
      - (결정) 이건 Thomas의 오해야. target에 따라서 EzServer가 다르게 분배를 해야 하는줄 알고 있었어. 우리는 Vatech-Target만 넣어서 모두 GW로 보내는 GW가 알아서 맞는 Target으로 보내는 것이고 답했어.
        - 그래서 이해해서 문제 해결.
    ...
    - Web Frontend App들의 경우,
      ...
      - Vatech-Clinic-Id 값은 관련된 일부 Backend 들만 가질 수 있고, Frontend, Desktop App들은 자체적으로 설정할 수 없다.
        - (?)EzWebServer API를 사용하면 가져올 수도 있지만 그 정도로 반드시 넣어야 하는 값인가?
          - (결정) 반드시 넣어야 한다.
    - Raymond 답변
      - Browser User-Agent 변경 불가
        - → GW: GW는 머신 판정(버전 게이트·SW 인벤토리)에 User-Agent를 쓰지 않고 전용 헤더(Vatech-Product/Version/OS/Clinic-Id/Via)로 합니다. 우리 전용 헤더가 이미 있어 새로 정의할 것이 없고, 브라우저의 UA 통제 제약과 무관합니다. SRS에 남은 "표준화된 User-Agent 필수" 문구는 삭제하고 UA는 로그·관측용으로만 두겠습니다.
      - Vatech-OS 획득 불가
        - → GW: OS는 best-effort로 둡니다. 있으면 보내고 없으면 빈칸 또는 unknown으로 처리하며, GW는 OS로 게이팅하지 않고 인벤토리에 unknown으로 저장합니다. SRS에서 OS를 필수에서 선택으로 완화하겠습니다.
      - Vatech-Clinic-Id 자체 설정 불가
        - → GW: Clinic-Id는 클라이언트가 아니라 클리닉-바운드 홉(EzServer)이 주입합니다. GW는 서명된 device 토큰으로 테넌트를 확정하고 Clinic-Id는 교차검증용이므로 Frontend·Desktop이 몰라도 됩니다. SRS 헤더 규약에 "EzServer 주입"으로 명확화하겠습니다.
        - (결정) Nginx proxy를 통하면 Vatech-Clinic-Id를 주입하기 어렵다.
          - 방안을 찾아본다. 전규현/ Raymond
          - 확장을 할지, 설정만으로 할지, 별도의 process로 할지.
  - 추가 Raymond 의견
    - 선결 확정 1건: 웹 프론트엔드가 GW로 직접 요청을 originate하지 않고 클리닉-바운드 홉(EzServer/백엔드)을 경유하는 것이 맞는지 확정 부탁드립니다(브라우저 SPA는 device 인증 경로 밖). 이것이 맞으면 위 세 건이 그대로 해소됩니다.
    - 클라이언트 식별 헤더 표준은 이제 GW SRS(7.7.1)가 정본입니다. 개발 Roadmap 문서는 더 이상 업데이트되지 않으니 참조하지 말아 주세요. 안건에 인용된 Roadmap 5장 대신 GW SRS를 기준으로 봐 주시면 되고, 클라이언트 클래스별 역량 등급(브라우저=UA·OS 면제)도 GW SRS에 반영하겠습니다.
...
5. **GW Console v1/v2 분리 · v1.0 최소기능 선행 · 기술 스택 확정 (R5).**
   ...
   - (반영) GW Console 백로그 **CB-2**. Console SRS 스택 표기 "권장·LLD 확정" → **"확정"**(§2.2·§3.4.2·Appendix C-4). _LLD 참고: shadcn은 Refine 1급 UI 통합이 아니라 Radix+Tailwind 직접 구성 → Refine은 headless로._
...
- 이번 주 진행 _(프레임 · 8/13 회의 시 확정 · 상세·수치는 아래 논의 R#/공유 S# 한 곳에만)_
  ...
  - **[GW Console]** SRS baseline v1.0 + **P0 완료**(10 Task) + **P1 3/9**(Entra 로그인·`/me` 부트스트랩·권한 매트릭스) _(상세 = 공유 S4)_
    - 전용 repo 스캐폴드~dataProvider·i18n·CI 게이트 · Entra OIDC(Auth Code+PKCE) · accessState 분기 · 역할×액션 매트릭스 게이팅
  - **[제품 연동 스펙]** EzServer OnePager 수령 확인 (잔여)
  - _(이번 주 결정사항 = 회의 시 추가)_

- 논의 사항 (이번 주 · 신규 논의/결정 안건)
  - **[논의·결정] AXS 대용량 파일 전송이 GW를 경유해야 할 수 있음 — GW 대역폭·비용 급증 우려 (제품 연동 스펙)**
    - **발견(확정):** AXS 파일 저장소(fss)는 **인증 게이트**다 — 업로드(`POST {storageUrl}/file`)에 **Bearer(AXS OAuth 토큰)+`Organization-ID`** 필요(AXS 샌드박스 Postman + 실 호출 캡처: `storageUrl`=`ppr-api-weu-fss.straumann.com`·**쿼리 서명 없음**). 즉 S3식 자기서명 presigned URL이 **아니다.**
    - **함의:** 클라(EzServer/CleverOne)는 AXS 토큰·Org-ID를 안 가짐(GW 중앙 보유) → 파일 바이트를 AXS에 **직접 못 올린다** → **GW가 인증 주입하며 프록시**해야 한다. 현행 SRS의 **"presigned 직결·GW 미경유"(§957·§236·§749 등) 전제와 충돌.**
    - **비용 리스크:** CT·스캔(수백 MB) 파일이 GW를 통과하면 **GW egress·대역폭 비용이 파일 총량(클리닉×케이스×크기)에 비례**해 커진다. 현행 GW는 제어 경로(메타)만 전제라 이 부담이 설계에 없음.
    - **⚠ 아직 확정 아님(재검토 필요):**
      - **① v1.0 실부담 경로(다운로드)는 미확정** — v1.0=IO Scanner. 스캔 **업로드는 스캐너가 AXS로 직접**(우리 GW 무관). 우리 부담은 **EzServer의 결과 스캔 다운로드**(`storageDownloadUri`)인데 실측 시 placeholder라 **n/a(값 못 봄)** → auth-gated인지 미확인(Postman상 auth 사용이라 개연성은 높음). **확정된 것은 문서 업로드(CleverOne 시나리오·v1.1+).**
      - **② AXS 문서 vs 샌드박스 불일치** — AXS OpenAPI 예시는 `storageUrl`을 서명 URL(`?signature=`)로 표기. 샌드박스는 서명 없는 fss. **Straumann에 "프로덕션이 auth-gated fss가 맞나·서명 URL 변형 없나" 확인 필요.**
    - **결정/조치 후보:** (a) §957을 "미경유→GW 프록시"로 변경 여부 · (b) Straumann에 **자기서명 presigned 지원 요청**(협상 여지·대형사 아님) 또는 EzServer 단기 스코프 토큰 위임(중앙화 완화) 검토 · (c) **비용 추정(파일량×단가) 선행** · (d) 우선 **Straumann 확인 + 다운로드 실측**으로 사실 확정.
    - **현재:** presign **발급 호출** 중계(T-E2E-12-5)는 완료·§957 정합(발급만 프록시). **바이트 경로만** 이 논의. 구현은 결정 전까지 현행(미경유) 유지.
...
- 공유 사항 (결정 아님 · 정보 공유 · 매주 상시)
  ...
  - **S1. 프로젝트 일정(Gantt) — 8/13 스냅샷** — 스펙 생애주기(작성→PR→baseline) + GW 구현 타임라인.
    - **진행률(구현)**:
      ...
      - **GW Console ≈ 25%**(IP Task 13/51 머지 — **P0 완료 · P1 3/9**)
    ...
    gantt
        ...
        section ③-C GW Console — gw/1.0 대응 v1.0 (구현 ~25%·P0 완료·P1 3/9 · frontend·별도 repo·전규현/Raymond)
  ...
  - **S4. GW Console(③-C) 현황 — frontend · 전용 repo (8/13)** _(GW 백엔드=S3와 분리 — repo·스택·세션 다름)_
    ...
    - **구현 착수(8/12)**: 별도 **frontend 세션** 오픈 완료 → **P0 완료(10/10) → P1 3/9(8/13)**. Task 단위 PR → 사람 머지(유인 모드·IP §7). **Entra/실 GW 없이 mock으로 대부분 진행 가능**(실배포 선결만 = C-2 Entra·C-10 도메인·C-3 CORS = ③-I/IT).
    ...
    - **상태 범례**(S3와 동일): 🔥 **이번주 완료**(8/6 회의 이후 main merge) · ✅ 이전 완료 · 🟠 진행중/착수예정 · ⬜ 대기 · 🔴 외부/인프라 선결. **표기 규칙(8/13)**: **이번주 완료(🔥)는 Task 단위**로 전개(진척 가시화) · **완료 Phase(지난주까지·전체 동일 상태)는 Phase 1행**으로 묶음 · **Phase 내 상태가 다른 Task만 별도 행**(예: P0-1) · 미착수 Phase = 1행.
      ...
      | ─ **대기·잔여** ─ |  |  |  |
      | **P1** 1-3·1-4·1-6~1-9 | App Shell 완성(리전 스위처·알림 badge)·역할별 홈·권한 요청/승인·운영자 관리 | 🟠 진행중 | **P1 3/9** · 다음=**1-3**(1-2·1-5 선결 충족) · risk:auth(1-9 last-admin 가드) · **P1 완료 시 로컬 GW 실데이터 확인 가능** |
...
- 논의 사항 (이번 주 · 신규 논의/결정 안건)
  - _(이번 주 결정사항 = 스펙 세션 정리 후 반영 · 회의 중 신규 안건 발생 시 여기 추가 · 보류·선결은 아래 「이월 논의 사항」 표 참조.)_
  - **R1. [결정] GW Console 접근 통제 = Entra(앱 계층) 단독 — 별도 네트워크 접근 제한 없음** — dev 프리뷰가 인터넷에 열려 있으나(8/19 실측·`console.gw.dev.ezcld.net`), **추가 보안 정책(사내 한정·IP allowlist/VPN/Basic)은 두지 않기로 결정**. 통제는 **실 Entra 빌드의 앱 계층 인증**으로만 한다.
    - **`infra-requests.md` §2 항목 3(접근 제한) = 철회** — Jack 회의 참석·인지(별도 통지 불요) · 요청서 문구만 정합 정리(선택).
    - **유의**: 현 **목(로그인 우회) 빌드**는 Entra가 없어 실 entra 빌드 전까지 화면 구조가 공개된다(목 데이터·**PHI 없음**·실 GW 미접속). 노출을 닫는 유일 수단 = **실 entra 빌드로 전환**(dev GW+Entra 접목 시 자동 종료). 추가 정책이 없으므로 그때까지 이 노출은 수용.
    - (결정) Entra로만 보안조치를 한다.
  ...
  - **[③-I Jack 인프라 요청 추적 · PR #12653]** — GW Console repo `infra-requests.md`로 Jack(③-I)·IT에 3건 요청. 회의에서 **Jack 상태·ETA 확인**(아래 #2 마감 경과). PR: https://dev.azure.com/ewoosoft/es-platforms/_git/vt-api-gateway-console/pullrequest/12653
    ...
    | 2-3 | ↳ **접근 제한** — **철회(R1 결정)**: Entra 앱 계층 단독·별도 네트워크 제한 없음 | ③-I | ✅ **불요(철회)** · Jack 회의 인지 | — |
...
- 공유 사항 (결정 아님 · 정보 공유 · 매주 상시)
  ...
  - **S1. 프로젝트 일정(Gantt) — 8/20 스냅샷** — 스펙 생애주기(작성→PR→baseline) + GW 구현 타임라인.
    ...
    gantt
        ...
        section ③-P-EZ EzServer 연동 스펙 (① 초안+PR=Raymond → ② Teddy 상세·리뷰·수정 → ③ baseline · IO Scanner부=보류)
        ① 초안+PR (Raymond·기본 GW연동) :done, ezw, 2026-07-20, 5d
        ② Teddy 상세작성·리뷰·수정 :active, ezpr, after ezw, 31d
        ③ baseline :milestone, ezbl, after ezpr, 0d

        section ③-P-CS CleverSpace OnePager (① 초안+PR=Raymond → ② CleverSpace팀(Larry) 상세·리뷰·수정 → ③ baseline)
        ① 초안+PR (Raymond·PR #12239·EzCloud) :done, cssub, 2026-07-27, 5d
        ② CleverSpace팀(Larry) 상세작성·리뷰·수정 :active, cspr, after cssub, 24d
        ③ baseline :milestone, csbl, after cspr, 0d

        section ③-P-CO CleverOne OnePager (① 초안+인계=Raymond·SharePoint → ② CleverOne팀(Nick) 상세·리뷰·수정 → ③ baseline)
        ① 초안+인계 (Raymond·SharePoint gw_adaptation) :done, cosub, 2026-07-27, 5d
        ② CleverOne팀(Nick) 상세작성·리뷰·수정 :active, copr, after cosub, 24d
        ③ baseline :milestone, cobl, after copr, 0d
...
- **이번 주 진행 (~9/17 회의) — 지난 주(9/10) 완료 요약 + 이번 주 착수**
  ...
  - **이번 주(9/17) 착수·진행 · 선결 대기**
    ...
    - **[GW Console 통합]** ⭐ **실 Entra + 제3자 전 경로 검증 통과(9/15)** — 권한 생명주기가 실 환경에서 한 바퀴 돌았다
      ...
      - ✅ **운영자 매뉴얼 반영(9/15)** — PR https://dev.azure.com/ewoosoft/es-platforms/_git/vt-api-gateway-console/pullrequest/14373
        - ⚠ **화면이 바뀌었는데 매뉴얼이 옛 화면을 설명**하고 있었다
          - `(이미 요청함)` 한 문구 → **`(이미 보유)`** / **`요청됨 · 승인 대기`** 로 갈렸다
          - 별도로 뜨던 "승인 대기" 화면은 없어지고 **행에 붙는다**
        - ⭐ **빠져 있던 경로 둘을 넣었다**
          - **케이스 F 신설** — 이미 쓰는 사람이 역할을 더 받는 길
            - ⚠ `내 역할` 진입점이 생겼는데 **매뉴얼 어디에도 없었다**
            - 사이드바가 아닌 이유도 적었다 — **역할이 모자라 요청하려는 사람에게는 사이드바 항목이 안 보인다**
          - **케이스 C 보강** — 회수 후 **재요청이 가능**하다
            - ⚠ 이 말이 없으면 회수당한 사람은 **끝난 줄 알고 포기**한다
            - 완전히 막으려면 **계정 정지(케이스 D)** 라는 것도 짚었다
        - 권한 요청 화면 **스크린샷 추가** — 가장 많이 바뀐 화면인데 그림이 없었다
        - ⭐ **매뉴얼은 화면이 바뀌면 같이 바뀌어야 한다** — 이번엔 PL 이 물어서 발견됐다

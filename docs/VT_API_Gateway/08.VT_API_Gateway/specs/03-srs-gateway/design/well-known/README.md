...
5. CI 파이프라인(**Azure Pipeline + AWS CLI**): YAML 스키마 검증 → env별 `server-configuration.json` 생성 → **AWS AppConfig 발행**(`create-hosted-configuration-version` + `start-deployment`·배포 전 JSON Schema 검증·점진·경보 자동 롤백·§7.7.5). 앱 build/deploy와 분리(`config/**` path-scoped).

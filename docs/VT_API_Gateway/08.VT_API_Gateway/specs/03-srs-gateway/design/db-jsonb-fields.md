...
- **표준 목록(초기 — 앱 레벨 상수, 확장 가능)**: `region.open` · `region.change` · `region.withdraw` · `device.approve` · `device.suspend` · `device.revoke` · `enroll.rotate` · `credential.issue` · `credential.rotate` · `policy.create` · `policy.update` · `policy.delete` · `orgmapping.upsert` · `target.upsert` · `target.delete` · `killswitch.toggle` · `config.publish` · `operator.status` · `operator.role.request` · `operator.role.grant` · `operator.role.decide` · `webhook.payload.view`(break-glass 본문 열람·§7.6.3).

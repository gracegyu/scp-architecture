...
- **모델 = RBAC(역할) + 데이터 스코프.** 역할은 **앱 레벨 상수**(`admin`·`developer`·`cs`·`operator`·확장형·DB enum 아님·db-jsonb#operator_role), **멀티 부여** 가능. 데이터 접근은 **스코프**(global/region/clinic·config/policy와 동일 다형)로 좁힌다. 역할→허용 action 매핑은 **앱 레벨**(별도 permission 테이블 없음). **level(등급) 방식 아님** — 역할은 서열이 아니라 직교 역량이라 RBAC가 맞다.

...
| `clinic` | **enroll 자동 upsert + LMP clinic 정보 포착(enroll 스냅샷·LMP 미sync·7/9 R3)**(§2.3.1) · 운영자 `POST /v1/admin/clinics`(예외) | device 자가 `PATCH /v1/clinics/me`(수동 보정)·`PUT /v1/clinics/me/region` / 운영자 `PATCH /v1/admin/clinics/{clinicId}`·`PUT /v1/admin/clinics/{clinicId}/region` | 하드 삭제 미지원 | 조회: device `GET /v1/clinics/me` · 운영자 `GET /v1/admin/clinics`(list)·`/{clinicId}` (§7.9·필드셋 확정=name·country_code·address·phone·website) |
...
- **[2] GW 온보딩 (EzServer Console → enroll)** — LMP Clinic-ID를 실어 `/v1/enroll/*` → clinic·device·region 확립 → **활성화**: (A안) C/S가 Console 승인(**v1.0 확정·7/9 R4**) / (B안) LMP 제3자 서명 자동승인(**v1.0 미지원 — LMP 재개발 후 적용**). 정본 = **§2.3.1**. (라이선스 등록 흐름에 enroll을 태워 설치자 개입 최소화 — 7/2 R9)
...
본 절은 이후 시나리오가 **공통으로 올라타는 토대**다 — GW의 프록시·분배 시나리오(§2.3.4 외부 연동·§2.3.5 파일 업로드·§2.3.6 Webhook)가 모두 이 라우팅 골격 위에서 동작한다. 대표 경로 **`CleverOne → EzServer → GW → AXS`**(외부 target 호출)를 예로, **구간마다 target을 어떻게 지시하는지**를 보인다(ADR-11 · 7/2 R1, 상세 §4.1.2·§4.5.1).
...
> **(B안) 제3자(LMP) 서명 자동승인 — LMP 라이선스 등록 device · v1.0 미지원 · LMP 재개발 후 적용 (7/9 R4 확정·Appendix B #42)** — 아래 flow는 스펙에 유지하되 v1.0 미구현
...
DNS 호스트는 *클라이언트가 접속하는 외부 계약*이므로 본 SRS에 기록한다. **GW API apex `gw.vatech.com`은 확정(Scott)** 이다. **GW edge 라우팅은 target 서브도메인 `{target}.gw.vatech.com`으로 한다(ADR-11 · 7/2 R1, §4.1.2)** — apex는 GW 고유 API(A), target 서브도메인은 등록 target 프록시(내부 B·외부 C). 인증서·GeoDNS 구성·리전 내부 호스트 네이밍 등 *구성*은 인프라/플랫폼팀 소유이며, 아래 표의 나머지 항목(리전 내부 호스트·Console·Webhook 경로)은 규약·예시다.
...
- **프록시 복원력(§7.5.4)**: **GW→target 연결 timeout(connect 3s/response 10s·AXS SLA/total_deadline<클라)은 GW 책임**(`target`, 7/2 R4·D1~D3). **재시도·서킷은 service mesh(istio) egress**(GW 미구현). GW는 추가로 오류 정규화·멱등·취소 전파. 수치=Appendix B #25
...
Webhook 전달 보증(QoS1·재시도·DLQ), 업로드 idempotency. **동기 프록시(내부·외부 target) 복원력**(7/2 R4): **GW→target 연결 timeout(connect/response/total_deadline)은 GW 책임**(자기 아웃바운드 호출 bound, §7.5.4). **재시도·서킷 브레이커로 장애 격리는 service mesh(istio) egress**가 담당. GW는 추가로 **오류 정규화·멱등·취소 전파**. MTBF 목표 TBD.
...
- **에러**: 갱신에 실패하면 백오프 재시도하고, 계속 실패하면 해당 target 호출을 차단하고 알람을 낸다. *(이 재시도는 **앱 레벨 토큰 수명 관리**이며, 프록시 요청 경로의 재시도·서킷=mesh(istio, §7.5.4·R4)와 별개다.)*
...
- **인가(authz) = GW 자체 (A방안·7/9 R2 확정).** Entra는 **SSO(authN)만** 담당하고, "누가 무엇을 볼 수 있나"(역할)는 **GW가 자체 관리**한다 — Entra App Role/Group에 인가까지 맡기면 운영 부담이 커 배제. GW에 **`operator`(신원)·`operator_role`(역할 부여)** 테이블을 둔다(**이전 "operator 무상태·user 테이블 없음" 방침 반전**). 역할 모델·승인 흐름=§7.9.2.
...
- **관측(§6.3.2 연계).** GW 로그/메트릭에 `targetLatencyMs`·`targetStatus`·`timeout`(GW 자기 timeout 여부)·`errorOrigin`(gateway|target)을 남긴다. **재시도 횟수·서킷 상태는 mesh(istio) 텔레메트리**가 정본이며, GW 로그와 함께 **Grafana Alloy로 수집**(§6.3.2·R3)해 원인을 식별한다.
...
**프록시(내부·외부 target) 오류 매핑(§7.5.4 연계).** **GW 자기 연결 timeout·연결 실패**(GW→target 아웃바운드, R4·D1~D3)와 **mesh 기인 서킷·일시불가**(istio egress)를 GW가 표준 envelope로 **정규화**한 것과, target이 _돌려준_ 오류를 구분한다 — 전자는 GW 표준 envelope, 후자는 verbatim 통과. `Vatech-Error-Origin` 헤더로 책임 주체를 표시한다.
...
> **원칙**: GW 정규화 오류(GW 자기 연결 timeout·연결 실패 + mesh 기인 서킷)만 GW envelope를 쓰고, target 응답은 코드·body를 **그대로 통과**(verbatim bypass 일관성, §4.1.2-3). 호출자는 `Vatech-Error-Origin`으로 "GW/인프라가 못 갔다(gateway)" vs "대상이 거부했다(target)"를 구분한다. **GW→target 연결 timeout은 GW 책임**, **재시도·서킷은 mesh(istio) 담당**이다(§7.5.4·R4).
...
**소유·성격 (7/9 R11 확정) — GW는 간이(interim), 정식은 신규 LMP.** 이 기능의 **정식 소유는 추후 개발되는 신규 LMP**다 — LMP는 클라이언트 **설치·업데이트** 기능이 있어 SW 수집·관리와 궁합이 맞다. GW에는 **간이(lightweight) 버전**을 넣는다(FR-COMPAT-01 헤더 재사용이라 저비용) — 신규 LMP 이전까지 클리닉별 버전 가시성 역할을 충분히 한다. **식별용 `Vatech-Instance-Id`는 도입하지 않고 일단 수집**한다(아래 튜플 모델).
...
**미래 — 정밀 식별(신규 LMP 소관).** 공용 라이브러리에 **`Vatech-Instance-Id`**(클라이언트 생성 안정 install GUID)를 도입하면 per-instance 인벤토리·**정확 대수**·정밀 업그레이드 추적이 가능하다. 단 **전 제품 클라이언트 변경**이라 표준 헤더 확장(① One Pager·③-P) 협의가 필요하다 — **7/9 R11에서 GW 미도입 확정**(GW는 식별 없이 일단 수집·튜플 모델 유지). **정식 per-instance 수집·관리는 신규 LMP**가 맡는다(설치·update 기능 궁합·§7.8.5 소유 note).
...
FR-ADM-02 (권한 분리). **7/9 R2 A방안: authN=Entra SSO / authz=GW 자체**(역할·부여·승인을 GW가 관리·§7.1.4). 역할별 수행 가능 기능 제한(§6.5).
...
| 25 | 프록시 복원력 값(연결 timeout·istio 정책) | GW+인프라 | 튜닝 **값**·istio 정책 — 분담 아키텍처는 §7.5.4(7/2 R4) 확정 |
...
| 아키텍처·라우팅 | ADR(특히 ADR-11 재평가(7/9 R5))·3-plane·§2 | **Thomas** (복수 아키텍트 추가 가능) |
...
| 2026-07-08 | **`/v1/admin/devices` API 리뷰 — OpenAPI↔DBML↔SRS↔flow 정합 + 누락 보강** — device 관리 API 전면 대조. **정합 확인**: lifecycle(pending→active→suspended→revoked)·kill(FR-FLEET-02=revoked+denylist)·no-delete(=revoked·하드삭제 없음)·nullable(clinicId·clientId)·region 파생(컬럼 없음·§6.4.1)·private_key_jwt(clientId+clientPublicKey)·FR 매핑(FR-DEV-01 CRUD·FR-DEV-03 lifecycle)·enroll 승인=`PATCH /v1/admin/devices/{id}` pending→active(§2.3.1 flow·provenance). **보강**: (1) **Device 스키마에 `createdAt`/`updatedAt` 추가** — DBML `device.created_at/updated_at`엔 있고 GET이 '시각' 약속했으나 스키마 누락이었음 · (2) `deviceId`·`DeviceId` param `format: uuid`(UUIDv7·§7.2.1) · (3) **`getAdminDevices`에 쿼리 파라미터 신설**(`status`·`clinicId`·`limit`·`cursor`) — 설명은 필터·커서 페이지네이션 약속했으나 미선언(getAdminFleet 패턴 일치) · (4) POST create body 필드 소유권 명확화(deviceId·clientId·status·시각=GW 생성). **잔여(별개)**: 다른 list op(getAdminClinics·Audit·OrgMappings·Upstreams)도 쿼리 파라미터 미선언 — 일괄 정합 별도 검토. redocly valid | (작성자 ID 미지정) |
| 2026-07-08 | **OpenAPI description 서식 정리 — 긴 단일행→멀티라인(의미 불변)** — 가독성: 긴 한 줄 description을 멀티라인으로. (1) 블록 레벨 `description: '…'`(>100자) **28건 → `>-` folded**(≈90자 wrap) · (2) 인라인 flow 프로퍼티 `prop: { type…, description: '…' }`(>120자) **30건 → block 확장 + description `>-` folded**. **의미 완전 불변 보증**: 변환 전후 YAML을 JSON deep-compare해 동일 확인(서식만 변경)·redocly valid·op 41. 스키마 값·enum·example 불변 | (작성자 ID 미지정) |
| 2026-07-08 | **OpenAPI 비-enum 파라미터에 example 전면 보강** — 파라미터 이해도 향상. **enum(status·state·scopeType)은 값 자명해 생략**, 그 외 전 파라미터(query·path)에 example 추가: clinicId=`d3f1…`(LMP 32자)·targetId/target=`axs`·externalOrgId=`'0040694997'`·eventType=`patient.created`·configKey=`gw.heartbeat.interval_seconds`·regionId=`apne2`·env=`production`·limit=`20`·**cursor=불투명 토큰 example+설명(직전 응답 nextCursor 그대로·직접 생성 금지)**. **getAdminDevices.clinicId=정확 일치(exact) 명시.** 정합 교정: webhook-events `from`/`to`가 `string date-time`이라 §1.3 Unix ms 규약 위반 → **`integer`(Unix ms)+example로 수정**. redocly valid·op 41 | (작성자 ID 미지정) |
| 2026-07-08 | **clinicId 예시 정정 — 추측 `CLINIC-0040694997` → LMP 32자 형식** — clinicId 예시가 근거 없는 추측이었음(사용자 지적): `CLINIC-` 접두는 창작, `0040694997`은 실은 **AXS organizationId** 예시를 오용. **근거 확인**(references `EzServer License Error Codes.md`): LMP `clinic_id`는 **정확히 32자**("clinic_id length is not 32 → 404")·형식은 32-hex로 보임(EzServer 설정 CSV 토큰 스타일). 전 산출물 예시 **23곳** `CLINIC-0040694997` → 32-hex `d3f1a9c07b6e4258af31c9d2e0b4a687`로 교체, clinic_id 정의부(DBML·OpenAPI `Clinic`)에 **'·32자'** 명시, §2.3.4 다이어그램 placeholder 정정. **실제 값·정확 형식은 LMP `licenseapi` 수신 기준**(예시는 형식 illustrative·#41 clinic record 연계). 날짜 changelog 이력의 옛 예시는 보존. redocly valid·DBML 컴파일 OK | (작성자 ID 미지정) |
| 2026-07-08 | **관리 조회 API 파라미터 정리 — getAdminDevices 블록화 + 필터 매칭 규약 명문화** — (1) `getAdminDevices` 파라미터를 인라인 한 줄 → **블록(멀티라인)** 로 정리(webhook-events 등과 통일·가독성). (2) **필터 매칭 방식 전수 검토·표시**: 전 관리 조회 API의 문자열·id·enum 필터는 **정확 일치(exact)** — 각 파라미터 description에 '정확 일치' 명시(webhook-events target/clinic/eventType/state·fleet clinic·config·policies scope/target 등). **full-text·부분 검색 없음**(v1.0). 예외 2종만 비-exact: **시각 범위** `webhook-events.from/to`(Unix ms 경계·description 추가)·**boolean 토글** `fleet.staleOnly`(필터 아님 명시). §7.9.1에 규약 문단 추가. redocly valid·op 41 | (작성자 ID 미지정) |
| 2026-07-08 | **관리 API(config·policy·upstream·region) 정합성 정리 — 파라미터 블록화 + timestamp 누락 보강** — (1) 남은 인라인 flow 파라미터 **12건**(config get/delete·policies get·upstreams `{targetId}`·policies `{id}`·regions `{regionId}`)을 **블록(멀티라인)** 로 정리(전 파라미터 블록 통일·인라인 0). (2) **DBML↔OpenAPI 필드 전수 대조**: `Policy`·`Upstream`·`Region` 스키마가 DBML엔 있는 **`created_at`/`updated_at`을 응답에서 누락**(Device에서 고친 것과 동일 부류) → 3개 스키마에 `createdAt`/`updatedAt`(int64 Unix ms·서버 설정·읽기 전용) 추가, `ConfigEntry`는 `updatedAt`만 있고 `createdAt` 비대칭 → `createdAt` 보강. (3) `Policy.id` example 불일치(스키마 `pol_axs_clinic0040`=스코프 인코딩 암시 ↔ path 파라미터 `pol_7f3a2b1c`=불투명) → **불투명 surrogate `pol_7f3a2b1c`로 통일**(id 설명에 'GW 생성·불투명 surrogate' 명시). enum(config_scope·policy_scope·upstream_profile·region_status)은 DBML과 전부 일치 확인. redocly valid·op 41 | (작성자 ID 미지정) |
| 2026-07-08 | **example/식별자 실형식 grounding 점검·교정(clinicId에 이어 2차 전수)** — 짐작·오형식 example 색출. **🔴 `externalOrgId`**: `0040694997`(10자리)은 AXS `webhooks.md` 구 샘플에 실재하나 실은 **customerNumber 형식** — AXS `organization.yml` 스키마는 `organizationId: format: uuid`이고 최신 `patientevents.md`도 UUID → **UUID로 교정**(실샘플 `e407b34d-c4b0-4db3-bbcd-cc11770eae7b`), OpenAPI 5곳·DBML 3곳·description 근거 명시. **🔴 잔존 창작값 제거**: `03c` 콘솔 _status의 `CLINIC-0040694997`(옛 clinicId 창작·교체 누락분) → 32-hex + org-id UUID. **🟡 GW surrogate id 탈-의미화**: `policy.id`=`pol_axs_clinic0040`(스코프 인코딩처럼 보임)·`config.id`=`cfg_gw_hb_interval_global`(key 인코딩) → **불투명 예시 `pol_7f3a2b1c`/`cfg_7f3a2b1c` + 'GW 생성·prefix/규칙 LLD·id에 scope/key 비인코딩' 명시**(OpenAPI Policy.id ↔ path 파라미터 불일치도 해소). **🟡 LMP/EzServer 발급값**: enroll `licenseProof`(LM-LIC…)·`serial`(EZ-SN…)에 '형식 illustrative·실형식 미확정' 명시. **`scopeId` 다형성 오해 소지**(clinic 32-hex를 범용 예시로): 파라미터 설명에 scope_type별 형식(global 생략/region regionId/clinic 32자/device UUID) 명시. Appendix B **#46 신설**(B-3·example 실형식 grounding·확정경로 7/9 R4·LMP/EzServer·LLD). 검증: redocly valid·op 41·DBML compile OK·잔존 짐작값 0 | (작성자 ID 미지정) |
| 2026-07-08 | **다형 `scopeId` 파라미터 설명 개선 — 단일 example → `examples`(복수·named)** — `scopeId`는 값 형식이 형제 `scopeType`에 종속(global 생략/region regionId/clinic 32자/device UUID)이라 **단일 example로는 오해 유발**(한 경우만 노출). config·policies 조회 3곳의 scopeId를 OpenAPI **`examples` 맵**(scopeType별 named 예시·UI 드롭다운)으로 전환·`schema.example` 제거. **2건 실오류 동시 교정**: (a) 지난 `replace_all`이 policies scopeId 설명에 'region=…'을 넣었으나 **`policy_scope`엔 region 없음**(global/clinic/device) → policy 설명에서 region 제거·"config와 상이" 명시 · (b) config scopeId 예시가 clinic 32-hex였으나 config는 v1.0 `gw.*` 키라 자연 스코프가 global/region(DBML도 `apne2` 사용) → config는 region/clinic/device 예시 제공(clinic은 policy용). 교훈=swagger-writer 가이드 반영(다형 파라미터→`examples` 복수·discriminator 리소스별 차이). redocly valid·op 41 | (작성자 ID 미지정) |
| 2026-07-08 | **clinic·device API 통합 검토 → 분리 유지로 기각(Appendix B #47)** — "현재 1:1이라 중복 아니냐·device로 통합할까"에 대한 아키텍처 판단. **기각·2 리소스 유지**: device·clinic은 SoT(GW UUIDv7 vs LMP 32자)·생성경로·lifecycle(device만)·관계역할(clinic=scope 앵커·org_mapping 라우팅 키·region 원천)이 상이하고, **1:1은 배포 우연이지 모델 제약 아님**(1:N·clinic-less #33·#19). 통합 정당조건 전부 불성립. **중복감 해소는 Console UI(③-C)에서** — device-중심 마스터-디테일·**편집면 단일화**(region/org-binding=clinic·lifecycle=device)·양방향 링크. 기록: Appendix B **#47 신설**(B-1 닫힌 결정), **③-C `_status`에 UI 구성 씨앗 섹션 추가**(콘솔 스펙 집필 시 승격). 선택 후속=관계 일급화(device 응답 clinic 임베드·`GET /admin/clinics/{id}/devices`)는 콘솔 스펙 시 확정 | (작성자 ID 미지정) |
| 2026-07-08 | **clinic↔device 관계 일급화 반영(#47 후속·op 41→42)** — 통합 대신 관계를 계약으로 노출. (1) **nested `GET /v1/admin/clinics/{clinicId}/devices`** 신설 — clinic의 device 하위목록(1:N 자연 표현·Clinic 상세 화면용). getAdminDevices와 동형(array of Device·status·limit·cursor)이고 clinicId=path. (2) **`Device.clinic` 요약 임베드**(object·nullable·readOnly — clinicId/name/region) — device 화면이 clinic 2차 조회 없이 컨텍스트 카드 렌더·**clinic-less(clinicId=null)면 null**·쓰기 시 무시. 정합: api-surface-matrix(42 ops·device·clinic 행)·§7.9 매트릭스·Appendix B #6(42)·#47(반영)·③-C `_status`(반영 완료) 동기화. redocly valid·op 42 | (작성자 ID 미지정) |
| 2026-07-08 | **kill-switch·DeviceStatus 설명 보강(의미 명확화·스펙 갭 해소)** — "kill이 EzServer를 끄나·revoke인가·재활성 되나" 질문 계기. **명확화 3점**: (1) kill=**GW 서비스 거부**이지 **EzServer 프로세스 정지 아님**(GW는 방화벽 뒤 device 접속 불가·§7.8.1) — device는 돌지만 GW 통한 호출·토큰·webhook 차단 · (2) kill=status=**revoked**+denylist 즉시 전파(§7.2.4) · (3) **revoked=종단(복구 불가)** — 재서비스는 되돌리기가 아니라 **재-enroll**(§7.2.7), 되돌릴 정지가 필요하면 **suspend**(`active↔suspended`). **스펙 갭 해소**: §7.2.3이 `pending→active→suspended→revoked` 선형만 서술 → **허용 전이 명시**(`active↔suspended` 복구 가능·`revoked` 종단)와 **kill vs suspend 구분** 추가. OpenAPI: kill op description 상세화(3단계)·**DeviceStatus enum에 값·전이 설명 신설**(기존 값 설명 없었음). **provenance 확인**: kill-switch는 원본 80 문서 실재(FR-FLEET-02·구 `POST /admin/v1/devices/{id}/kill`)·경로만 `/v1/admin/*` 재정렬. redocly valid·op 42 | (작성자 ID 미지정) |
| 2026-07-08 | **kill 설명 다듬기 + Console kill UI 씨앗(③-C)** — kill 설명에서 "EzServer 프로세스를 끄는 것이 아니다" 부정 서술 제거(불필요)·긍정 서술(GW 서비스 거부)만 유지(OpenAPI kill op 3→2단계·SRS §7.2.3 동일 정리). **③-C `_status`에 Device lifecycle 액션 UI 씨앗 추가** — 승인/suspend·resume/**kill-switch** 제공, kill=파괴적·비가역이라 **가드 필수**(확인 다이얼로그·사유·권한·감사 노출·되돌리기 없음·재-enroll 안내)·suspend와 시각 분리. redocly valid·op 42 | (작성자 ID 미지정) |
| 2026-07-08 | **`/v1/admin/clinics` API 리뷰 — OpenAPI↔DBML↔SRS↔flow 정합(device 리뷰와 동일 깊이)** — clinic 관리 API 전면 대조. **정합 확인**: `Clinic` 스키마↔DBML `clinic` **완전 일치**(clinicId·region·name·countryCode·address·phone·website·mappingVersion·createdAt·updatedAt 10필드·timestamp 포함·누락 없음) · POST=`ClinicRegistration`(clinicId+region·서버필드 미포함)·PATCH=`ClinicInfo`(info-only·region/clinicId 제외) **입력면 분리 적절** · region 변경=별도 `PUT …/region`(편집면 단일화·#47 일치) · device-self(`/me`)↔operator 평면 분리 정상 · provenance(구 80=`GET /v1/region/resolve` RGN-01/02 → clinic 표면 재구성·#4). **🔴 수정**: (1) **phantom `status` 제거** — `getAdminClinics`("상태 필터")·`getAdminClinicsById`("status 조회")·`getClinicMe`("국가·status") description이 clinic `status`를 언급하나 **DBML·스키마에 clinic status 없음**(device에서 복사된 유령 필드·A/B/C는 AXS 가입상태·④ 소관이지 clinic 필드 아님) → 제거 + "clinic은 lifecycle status 없음" 명시 · (2) **`getAdminClinics` 파라미터 누락 선언** — 설명은 필터·페이지네이션 약속했으나 `parameters` 미선언(device getAdminDevices와 동일 버그) → `region`(정확 일치)·`limit`·`cursor` 신설. **🟡**: FR 매핑 구체화(`FR-RGN-*` → clinic registry=`FR-RGN-06`·read-back=`FR-RGN-01`·region=`FR-RGN-04`, 5개 op). **관찰(LLD)**: info-only PATCH가 `mapping_version`을 bump하는지 애매(§7.3.2가 region 드리프트용인지 행 버전인지) — LLD 확정. redocly valid·op 42 | (작성자 ID 미지정) |
| 2026-07-08 | **`GET /v1/admin/audit` 리뷰 — 목록 조회인데 description 오류 + 파라미터 전무 교정** — audit_log 조회 API 대조. **🔴 발견**: (1) description이 이 엔드포인트를 설명하지 않음("그 외 관리자 조회는 LLD…" — 딴 얘기·오배치) · (2) `array of AuditLog`를 반환하는 **목록인데 필터·페이지네이션 파라미터 전무**(device·clinic getList와 동일 버그)인데 DBML은 `ts·actor·action` **인덱스**까지 있어 조회 축 명백. **수정**: description 재작성(조회 전용·필터+커서·IEC 감사) + **파라미터 7개 신설** — `actor`·`action`(정확 일치)·`result`(enum success/denied/failure)·`from`/`to`(Unix ms 범위)·`limit`(기본 50·최대 200)·`cursor`(webhook-events 패턴 일치·필터 축=DBML 인덱스 정합). `AuditLog.result`도 string→enum. §7.9.3에 조회(read-only) 서술 추가. **별건 관찰(미해결)**: §7.9.3은 감사 기록에 `before/after·IP`를 명시하나 **DBML audit_log·AuditLog 스키마엔 해당 컬럼 없음**(id·ts·actor·action·result 5개뿐) — SRS↔DBML 불일치·별도 판단 필요. redocly valid·op 42 | (작성자 ID 미지정) |
| 2026-07-08 | **audit_log에 `before_state`/`after_state`·`source_ip` 추가(A안) — §7.9.3↔DBML 불일치 해소** — 직전 리뷰서 발견한 "§7.9.3은 before/after·IP 기록을 명시하나 audit_log·스키마엔 없음"을 **A안(필드 추가)** 으로 온전 반영. **DBML** `audit_log`에 `before_state`·`after_state`(jsonb·부분 스냅샷·nullable)·`source_ip`(varchar·nullable) 컬럼 + 헤더 jsonb 목록 갱신 · **OpenAPI** `AuditLog`에 `beforeState`·`afterState`(object·nullable)·`sourceIp`(nullable) · **db-jsonb-fields#audit_log** 섹션 확장(문자열 규약→+jsonb, before/after shape·update=둘/create=after/delete=before·no-op 미기록·source_ip 규칙) · **§7.9.3 Side Effect** 실제 필드명·PHI 금지 명시. **PHI·원문 secret 금지**(관리 리소스 값만·참조/마스킹·§6.4)가 핵심 제약. DBML compile OK·redocly valid·op 42 | (작성자 ID 미지정) |
| 2026-07-08 | **`/v1/admin/org-mappings` API 리뷰 — 목록 파라미터·override 모순·스키마 parity·오태그 교정** — org_mapping 관리 API 전면 대조. **🔴 수정**: (1) `getAdminOrgMappings`가 목록인데 **파라미터 전무**(반복 버그) → `targetId`·`clinicId`·`externalOrgId`(정확 일치·DBML 인덱스 정합)·`limit`·`cursor` 신설 · (2) **`OrgMapping` 스키마 `createdAt`/`updatedAt` 누락**(DBML엔 있음·Policy/Upstream/Region과 동일 부류) → 추가 · (3) `postAdminOrgMappings`가 "교정(override)"이라면서 **409 Conflict** 둬 모순(override면 충돌 안 남) → **upsert로 정정**(존재 시 override=200·신규=201·409 제거·mapping_version++ 명시) · (4) **오태그 FR 교정**: postAdminOrgMappings `FR-RGN-04`(리전 재지정)는 org-mapping과 무관 → **FR-INT-***; getAdminOrgMappings·delete는 FR 태그 없음 → **FR-INT-*** 부여(device-self org-binding과 도메인 일치). **의도적 비대칭 유지**: operator POST=upsert(override 가능)·device-self `postClinicMeOrgBindings`=create-only(409·타 clinic 매핑 하이재킹 방지·교정은 operator). **정합 확인**: OrgBinding(self body)=targetId+externalOrgId(clinicId=토큰 self·적절)·DELETE 복합키(targetId+externalOrgId) query·OrgMapping required 적절. redocly valid·op 42 | (작성자 ID 미지정) |
| 2026-07-08 | **`/v1/admin/webhook-events` API 리뷰 — nullable 정합·필드 완성(파라미터는 이미 모범)** — webhook_event 조회 API 대조. **정합 확인**: `WebhookEvent`↔DBML **13필드 완전 매핑**·state enum(received/dispatched/dead_letter) 일치·목록 파라미터(targetId·clinicId·eventType·state 정확 일치·from/to 범위·limit/cursor) **이미 완비**(타 op의 패턴 기준)·PHI-free(payload 미포함·payloadRef claim-check). **🔴 수정(nullable 불일치·lifecycle상 늦게 해석되는 필드가 non-null이라 클라이언트 파손 위험)**: `externalOrgId`·`region`·`payloadRef`를 **nullable**로(DBML nullable·미해석/미보관 시 null) · `dispatchedAt` nullable+설명+예시 완성(미분배 시 null) · `required`에 항상 존재하는 `attempts`·`receivedAt` 추가 · **DBML `received_at`에 `[not null]`** 부여(insert 시 항상 설정·OpenAPI non-null과 정합). summary에 `FR-WH-*` 태그. **관찰(옵션)**: 단건 조회 `GET …/{eventId}` 없음(v1.0=search-only·api-surface-matrix 명시)·필요 시 후속. redocly valid·DBML OK·op 42 | (작성자 ID 미지정) |
| 2026-07-08 | **webhook 이벤트 단건(메타) 조회 신설 + payload 경계 재확인 — `GET /v1/admin/webhook-events/{eventId}`(op 42→43)** — "단건 조회로 payload까지 보나?"에 대한 판단. **payload는 단건으로도 제공 안 함**: opaque·**환자 PHI 포함 가능**이라 설계상(§6.4·§7.6.3) 관계형 DB 미저장·Console 미서빙·소비자=클리닉 EzServer(MQTT 하행·§7.6.6)뿐 → 단건 조회는 **메타 전용**(payload 열람은 별도 break-glass·redact·감사 경로·#36). **추가 이유**: 목록 필터에 eventId가 없어 특정 이벤트를 id로 조회(DLQ/장애 triage)하는 길이 없었음 → `GET …/{eventId}`(WebhookEvent 메타·404 포함) 신설, description에 payload 미제공·PHI 경계 명시. api-surface-matrix(43 ops·webhook_event read by-id)·§7.9 FR-ADM-01·Appendix B #6(43) 동기화. redocly valid·op 43 | (작성자 ID 미지정) |
| 2026-07-08 | **webhook payload 열람 = GW 중개 break-glass 신설(직접 S3 금지 명문화·op 43→44)** — "payload를 Console이 S3에서 직접 보나? 인증 문제?" 질문 해소. **Console 직접 S3 금지**(브라우저 IAM 불가·RBAC/감사/리전 주권/redact 우회) → **GW 중개만**. `GET /v1/admin/webhook-events/{eventId}/payload` 신설 — GW(리전 로컬)가 `payload_ref`로 S3에서 읽어 **PHI redact 후 반환**, **break-glass RBAC**(일반 operator 403)·**전량 감사**(action=`webhook.payload.view`)·TTL 만료 시 404. `WebhookPayloadView` 스키마(eventId·redacted·payload·retrievedAt) 신설. **경계 명확화**: 목록·단건 **메타**(`/webhook-events`·`/{eventId}`)=PHI-free·payload 미포함 / 본문=이 break-glass 1곳만. **정합**: §7.6.3(Console 상세 payload 열람이 약속만 있고 API 없던 갭 해소·GW 중개·직접 S3 금지 명문화)·§7.9 FR-ADM-01·api-surface-matrix(44 ops)·Appendix B #6(44)·db-jsonb 감사 action 목록(+webhook.payload.view)·③-C `_status`(직접 S3 금지 씨앗) 동기화. redact 필드·TTL=#36. redocly valid·op 44 | (작성자 ID 미지정) |
| 2026-07-08 | **webhook 수신(ingress) 엔드포인트 리뷰 — `POST /v1/webhooks/{target}`(마지막 미검토분)** — 운영자 조회 3개(events search/by-id/payload)에 이어 **실제 수신 API** 대조(FR-WH-01~05·§7.6·AXS webhooks.md 근거). **정합 확인**: `webhookHmac`(X-Signature·예시·upstream별 `sig_scheme` 상이) 적절 · target=레지스트리(inbound_host) 기반·미등록 404 · Host/SNI 식별·source IP 옵션. **수정**: (1) 응답 **`200`→`202 Accepted`** — 처리를 내부 비동기 큐로 위임(FR-WH-04)하므로 async accept 의미가 정확(AXS는 "acknowledge"=2xx면 OK·재시도 3회→DLQ) · (2) **`WebhookEnvelope` 논리(추출) 필드 명확화** — 실제 인바운드는 upstream 원 스키마(AXS=`messageId`≠eventId·`eventType`·flat `organizationId`)이고 GW가 `event_id_path`/`event_type_path`/`org_id_path`로 추출함을 명시(envelope는 illustrative·확정 계약 아님) · (3) 중복 eventId=2xx 멱등(FR-WH-03) 명시 · (4) `WebhookAck` required[received]. **webhook API 리뷰 완료**(수신 1 + 운영자 조회 3). redocly valid·op 44 | (작성자 ID 미지정) |
| 2026-07-08 | **`/v1/admin/fleet` API 리뷰 — phantom "버전" 실체화 + clinicId·online 노출 보강** — fleet 대시보드 조회 대조. **정합 확인**: `FleetState`↔DBML(deviceId·lastHeartbeat·successRate 일치)·필터(clinicId·staleOnly·limit·cursor) 완비. **🔴 phantom "버전"**: getAdminFleet 설명은 "성공률·버전 조회"·heartbeat도 `appVersion`을 롤아웃 가시성용으로 보고하는데 **fleet_state·FleetState엔 버전 필드 없음**(보고받고 버림) → **DBML `fleet_state.app_version` + `FleetState.appVersion` 신설**(heartbeat마다 갱신·nullable). **🟡 보강**: (1) `FleetState.clinicId`(device.clinic_id 조인·파생·nullable) — clinicId 필터는 있으나 응답 row에 clinic 미노출이던 갭(대시보드 clinic 그룹) · (2) **`FleetState.online`(파생 boolean)** — 서버가 staleOnly 필터용 임계 판정을 하면서 per-row online을 안 돌려주던 것 → 노출(staleOnly=online:false) · (3) getAdminFleet summary FR-FLEET-01/03 태그 · (4) postFleetHeartbeat·§7.8.1에 successRate·appVersion을 fleet_state에 영속함 명시. redocly valid·DBML OK·op 44 | (작성자 ID 미지정) |
...
| 2026-07-09 | **`/v1/admin/upstreams` API 리뷰 — upsert 409 모순·DELETE 종속성·단건 조회·오타 교정(op 45→46)** — 연동 대상 관리 API 대조. **정합 확인**: `Upstream`↔DBML 19필드(createdAt/updatedAt 포함)·enum(profile)·**list 무페이지네이션=OK**(소규모 카탈로그·v1.0 1개·regions와 동일 패턴)·credential/secret KMS 참조만(원문 미노출). **🔴 수정**: (1) POST가 "등록/갱신(upsert)"인데 **409 Conflict** 모순(org-mappings와 동일) → **upsert 200/201·409 제거**(PK targetId 있으면 갱신) · (2) summary 오타 `( 리)` → `(upsert)` · (3) **DELETE 종속성 미정의** — `org_mapping`·`policy`·`webhook_event`가 targetId FK 참조인데 삭제 시 처리 없음 → **참조 시 409**(먼저 매핑/정책 정리·orphan 방지) 추가 · (4) **단건 조회 `GET /v1/admin/upstreams/{targetId}` 신설** — 기능상 목록으로 충분하나(1~3개) **API 일관성·Console 편집 폼 프리필**용(다른 리소스와 동일·path 이미 존재). POST/DELETE에 FR 태그(FR-INT-01/02·FR-WH-01/02)·GET list 설명 보강(소규모 카탈로그 근거). api-surface-matrix·Appendix B #6(46) 동기화. redocly valid·op 46 | (작성자 ID 미지정) |
| 2026-07-09 | **7/9 R6 결정 반영 — 엔티티/용어 `upstream` → `target` 전면 리네임** — 마케팅·CS가 이해하기 쉬운 용어로 연동 대상 엔티티명을 **`target`으로 확정**(7/9 R6). 이미 키=`target_id`·헤더 `Vatech-Target`·서브도메인 `{target}`이라 **용어 통일** 효과. **전면 반영**: DBML `Table upstream→target`·`Enum upstream_profile→target_profile`·FK `> target.target_id`(policy·webhook_event·org_mapping) / OpenAPI `Upstream→Target` 스키마·경로 `/v1/admin/upstreams→/targets`·op(`getAdminTargets`·`postAdminTargets`·`getAdminTargetsByTargetId`·`deleteAdminTargetsByTargetId`)·프록시 에러(`TargetUnreachable/Timeout/Unavailable`·`TARGET_*`·`Vatech-Error-Origin: target`)·헤더 `VatechTargetLatencyMs`·감사 action `target.upsert/delete` / redis `gw:cache:target:{targetId}` / db-jsonb `## target`·앵커 / api-surface / ③-C·③i seed / SRS 본문·Appendix B. **PK 컬럼 `target_id`는 유지**(자연키=Vatech-Target 값). **history(날짜 changelog·ADR/결정 로그·DBML 날짜 주석)는 원문 보존**(SRS 30·DBML 7·db-jsonb 4행 upstream 잔존=의도). 검증: redocly valid·op 46·DBML 11 테이블 compile OK·**잔존 live upstream 0** | (작성자 ID 미지정) |
| 2026-07-09 | **7/9 R2 결정 반영 — Console 인가(RBAC) = GW 자체(authN=Entra SSO만·op 46→53·DBML 11→13)** — A방안 확정: Entra는 **SSO(authN)만**, 인가(authz)는 **GW 자체**(Entra에 인가까지 맡기면 운영 부담↑). **이전 "operator 무상태·user 테이블 없음"(§7.1.4·§7.9.2·/me claim 에코) 전면 반전.** **DBML 신설**: `operator`(JIT·SSO 신원·status active/suspended·비번 없음)·`operator_role`(RBAC 역할+스코프·요청→승인/거부/회수 lifecycle)·enum `operator_status`·`grant_status`. **역할=앱 레벨 상수**(admin/developer/cs/operator·확장형·DB enum 아님·db-jsonb#operator_role)+**스코프**(global/region/clinic·CS=global 전 클리닉). **OpenAPI 7 신설**: `POST /me/access-requests`(본인 멀티 요청)·`GET /operators`·`GET/PATCH /operators/{id}`(상태)·`POST /operators/{id}/roles`(직접 부여)·`PATCH …/roles/{grantId}`(승인/거부/회수)·`GET /access-requests`(승인 큐) + `GET /me` **DB 조회로 재작성**(claim 에코 아님·accessState active/no_access/suspended)·스키마(Operator·RoleGrant·AccessRequestCreate 등). **흐름**: 직원 전원 SSO 로그인→JIT operator(역할0=no_access·권한 요청 화면)→멀티 요청→Admin 승인 큐→승인(active)/거부(rejected·보존·빈 화면). 오프보딩=Entra 비활성+suspended. redis `operator-roles` 캐시·§7.1.4/§7.9.2 재작성·③-C RBAC 화면 씨앗·Appendix B #38 확정. redocly valid·op 53·DBML 13 compile OK | (작성자 ID 미지정) |
| 2026-07-09 | **운영자 역할 = 소스코드 enum 확정(role 테이블 기각·rollback 후 재정리)** — "role을 DB 테이블 vs 소스코드 enum?" 분석 결과 **소스코드 enum**(=DBML `operator_role_type` Enum→Prisma→TS) 채택. 근거: 역할→권한 매핑이 코드라 새 역할은 어차피 대대적 코드 변경 + DB 행만 추가하면 권한 0 유령 역할 → 테이블의 런타임 확장 이점 무력 + DB↔코드 드리프트. 이전에 넣었던 `role` 카탈로그 테이블·`GET /roles`·`Role` 스키마는 rollback(테이블 13·op 53 유지). **`operator_role.role` varchar 상수 → `operator_role_type` enum**, OpenAPI role 필드 enum[admin/developer/cs/operator]·§7.9.2·db-jsonb#operator_role·③-C 정리. **역할 추가 = enum + 소스코드(GW 권한매핑 + GW Console UI) + DB enum 동시 수정**(마이그레이션·양측 릴리스·런타임 무릴리스 추가 없음). redocly valid·op 53·DBML 13 | (작성자 ID 미지정) |
...
| 2026-07-09 | **7/9 R11 결정 반영 — 클라 SW 인벤토리 = GW 간이(interim) 확정 · 정식은 신규 LMP · Vatech-Instance-Id 미도입 확정** — 결정: **이 기능을 GW에 넣되 간이(lightweight)로**, 정식(중앙 수집·관리)은 **추후 개발되는 신규 LMP** 소관(LMP는 설치·**update** 기능이 있어 궁합)·GW 간이본은 신규 LMP 전까지 충분히 역할. **식별용 `Vatech-Instance-Id`는 미도입 확정**(식별 없이 일단 수집·(clinicId,product,version,os) 튜플 presence 모델 유지). **근거(EzServer Scan 맥락)**: EzServer는 이미 Scan+HW 고유정보로 클리닉 내 client PC를 식별 수집하나 **중앙 집중 관리가 없고**, 그 식별정보가 있어도 **GW 호출 attribution이 어려워** 현재 활용 곤란 → GW는 instance-id 없이 튜플로 간이 수집. **기능 골격(§7.8.5·FR-FLEET-06·`client_inventory`·`GET /v1/admin/clinics/{clinicId}/clients`·Redis seen-set throttle·③-C 화면)은 이미 반영**돼 있어, 본 결정은 **소유·성격(간이/LMP)·Instance-Id 미도입을 확정·명문화**. 문서: §7.8.5(소유·성격 note+EzServer Scan 참고+미래 신규 LMP 소관)·Appendix B #48(미도입 확정·LMP)·③-C(미래=미도입 확정·간이 뷰)·③-P-LMP seed(별도·미래 포인터). redocly valid·op 45·DBML OK | (작성자 ID 미지정) |
| 2026-07-09 | **7/9 전략 조정 — v1.0 AXS 연동 범위 = Straumann IO(IntraOral) Scanner만 (CleverOne 연동 post-v1.0) + 담당·Gantt 재정리** — 시간 제약상 AXS 연동(갈래A) v1.0 1차 범위를 **Straumann IO Scanner로 한정**하고 **CleverOne 연동은 v1.0 제외→후속 버전**으로 이관. **GW 기본 계약(COMPAT 게이팅·인증·라우팅·target 프록시)은 originator 무관 공통 토대라 v1.0 포함**. **IO Scanner↔EzServer 연동 방식 미정**(추후 확정). 제품 스펙 초안 주체 변경: **CleverOne=Nick·EzServer=Thomas**(GW는 표준 계약만 제공·Raymond 초안 미작성·SectionView 병행 부분투입). **목표=10월 출시**(역산·잠정). **문서 반영**: SRS §1.2(우선 개발 bullet)·§2.7(버전표 AXS connector 한정 qualifier + 범위 note) / Agenda 2개 Gantt(IO Scanner 우선·10월 역산·담당·deferred 섹션)·주변 불릿·제품×단계 표 note / 정본 **Roadmap §3.9 gantt+scope note 동기화** / ④·③-P-EZ(Thomas)·③-P-CO(Nick·deferred) seed. redocly valid·DBML OK(스키마 무변경) | (작성자 ID 미지정) |
| 2026-07-09 | **`/v1/admin/policies` API 리뷰 — upsert 규약·server-gen id 반환·DBML unique·페이지네이션 정합** — 정책 관리 API 전면 대조(OpenAPI↔DBML↔db-jsonb↔SRS↔matrix). **정합 확인**: `Policy` 스키마 8필드 ↔ DBML `policy` 8컬럼 완전 매핑(id·scopeType·scopeId·targetId·allowedEndpoints·scopes·created/updatedAt)·enum `policy_scope{global,clinic,device}` 일치·jsonb shape(allowed_endpoints method/pathPattern·scopes) db-jsonb 일치·egress 부재(=target.egressAllowlist·#31)·deny-by-default. **🔴 수정**: (1) **POST upsert가 `409 Conflict`+`201`만** — target/org-mapping upsert 규약과 모순 → **200(갱신)/201(생성)·409 제거**(자연키=(scopeType,scopeId,targetId)) · (2) **POST 응답 바디 없음**인데 `id`=GW 생성 surrogate라 클라가 알 수 없음(by-id 조회 없음·DELETE는 id) → **결과 Policy(id 포함) 반환**(200/201·target은 id=자연키라 불요했으나 policy는 surrogate라 필요) · (3) **DBML 자연키 인덱스 `[unique]` 누락** — 동일 패턴 config·client_inventory·operator_role은 unique인데 policy만 plain → **`[unique]` 부여 + global(scope_id NULL) 중복 미차단 LLD 주의**(config와 동일 NULLS NOT DISTINCT) · (4) **audit action=policy.update만** 언급 → **policy.create(신규)/policy.update(갱신)**(db-jsonb 표준목록 정합·before/after 규칙) · (5) **getAdminPolicies 페이지네이션 없음**(operator list 원칙 위반·clinic/device-scope는 fleet 규모로 증가) → **limit/cursor 추가**. **관찰(무변경)**: POST 입력=`Policy` 재사용은 target 선례와 일치(별도 입력 스키마 안 만듦·server 필드는 무시/설정) · read=list-only(by-id 조회 없음)는 matrix 명시 의도. api-surface matrix 동기화. redocly valid·op 53·DBML OK(CREATE UNIQUE INDEX policy) | (작성자 ID 미지정) |
| 2026-07-09 | **`/v1/admin/regions` API 리뷰 — 운영자 read 엔드포인트 누락 신설 + isDefault 불변식·DELETE 종속성·audit·PUT 경로권위** — 리전 카탈로그 관리 API 전면 대조(OpenAPI↔DBML↔SRS↔matrix). **정합 확인**: `Region` 스키마 7필드 ↔ DBML `region_catalog` 7컬럼 완전 매핑(regionId·displayName·endpoint·status·isDefault·created/updatedAt)·enum `region_status{active,draining,planned}` 일치·`clinic.region` FK 참조. **POST=create(409 중복)·PUT=update(by regionId)** 는 upsert가 아니라 create+update 모델이라 409 정당(policy/target upsert와 다름·의도). **🔴 수정**: (1) **운영자 read 엔드포인트 부재** — 공개 `GET /v1/regions`는 **deviceAuth 전용**이라 operator(Console)가 리전을 조회·관리 대상 선택할 수 없었음(admin 평면에 GET 없음·matrix는 'operator list ✅'라 주장하나 실제 없음) → **`GET /v1/admin/regions`(operatorAuth·list) 신설**(op 53→54). **🟡 수정**: (2) **`isDefault` 다중 true 방지 불변식 부재** → '정확히 1개만 true·새 지정 시 기존 해제' 명문화(OpenAPI 스키마·DBML 주석·부분 유니크 LLD) · (3) **DELETE 409 트리거 미문서화** → `clinic.region` 참조 시 409·default 리전 삭제 불가 명시 · (4) **admin 엔드포인트 audit action 부재** → POST=`region.open`/PUT=`region.change`/DELETE=`region.withdraw` 주석 + db-jsonb 표준목록에 region.open·region.withdraw 추가 · (5) **PUT 경로 vs 본문 regionId 모호** → 경로 권위·불일치 400 명문화. SRS §7.9.1·provenance 표·api-surface matrix 동기화. redocly valid·op 54·DBML OK | (작성자 ID 미지정) |
| 2026-07-09 | **전면 무결성 감사(SRS↔DBML↔OpenAPI↔design 교차검증)** — 오늘 대량 변경(R7~R11·IO Scanner·AppConfig·4-way·policy/regions 리뷰) 후 conflict 점검. **검증**: redocly valid·dbml2sql OK·SRS fence 52. **카운트 실측**: OpenAPI 54 ops·DBML 13 테이블·10 enum. **enum 10개 전부 DBML↔OpenAPI 일치**(device_status·webhook_state·target_profile·region_status·policy_scope·config_scope·operator_status·grant_status·operator_role_type·audit_result=success/denied/failure). **스키마↔테이블 커버리지 완전**(13 테이블 전부 대응 스키마·orphan 0). **§7.9.1 admin 엔드포인트 전부 실재**(getAdminRegions 통합). **잔재 0**: R7 payload-S3·R9 compat-S3/Secrets/gw:cache:compat·R10 3-way/두부분/bare Admin API·R6 live upstream 모두 이력행 외 없음(rename 완료). **발견·수정**: 스테일 op 카운트 2건(api-surface 헤더·Appendix B #6의 '53 ops' → 54). 그 외 conflict 없음 | (작성자 ID 미지정) |
| 2026-07-10 | **RBAC API 리뷰(`/v1/admin/operators`·`operators/{id}`·`/roles`·`/roles/{grantId}`·`access-requests`·`me`) — 7/9 R2 신규분 정식 감사** — operator·operator_role 관리 API 전면 대조(OpenAPI↔DBML↔db-jsonb↔SRS↔matrix). **정합 확인**: `Operator` 7필드↔DBML `operator` 7컬럼(+roles 임베드)·`RoleGrant` 9필드↔`operator_role` 10컬럼(operatorId=부모 컨텍스트라 제외 정상)·enum 3종 일치(operator_status·grant_status·operator_role_type)·scope_type varchar{global,region,clinic}(OpenAPI enum이 더 엄격·정상). **POST roles·access-requests의 409는 create(중복) 의미라 정당**(upsert 아님). getAdminOperators/access-requests **페이지네이션 有**. access-requests 큐=`Operator[]`(operatorId=Operator.id·grantId=roles[].id로 승인 가능). **🟡 수정**: (1) **감사 action drift** — db-jsonb 표준목록에 `operator.status`/`operator.role.grant`/`operator.role.decide`가 **누락**(OpenAPI·SRS §7.9.2는 참조 중) → 3종 + `operator.role.request` 추가, access-requests POST에 `operator.role.request` 주석·SRS §7.9.2 목록 보강 · (2) **operator_role unique 인덱스 NULL 처리** — global 역할은 scope_type/scope_id=NULL이라 표준 UNIQUE로 중복 미차단(PG NULL distinct·policy/config와 동일) → NULL caveat + scope_type 'NULL→global' 정규화 노트 추가. **관찰(무변경)**: grant POST 무바디(grantId=queue/GET로 발견·regions와 동일)·operator_role은 created/updated 대신 requested_at/decided_at lifecycle 타임스탬프(의도)·역할 전이 유효성=앱레벨. redocly valid·op 54·DBML OK | (작성자 ID 미지정) |
| 2026-07-10 | **`/v1/admin/config` API 리뷰(중앙 Config·FR-CFG-01·§7.8.4)** — config 관리 API 전면 대조(OpenAPI↔DBML↔db-jsonb↔SRS↔matrix). **정합 확인**: `ConfigEntry` 8필드↔DBML `config`(surrogate `id`만 API 미노출=natural-key 일관·policy보다 깔끔)·enum `config_scope{global,region,clinic,device}` 일치·`config_value`=jsonb(키 레지스트리 검증·PHI 금지)·**unique 인덱스 NULL caveat 이미 有(원조)**·PUT/DELETE 모두 자연키(scopeType,scopeId,configKey). **🟡 수정**: (1) **`updatedBy` 스푸핑 위험** — audit actor인데 읽기전용 표기 없어 클라 입력 가능 → **서버가 operatorAuth subject에서 설정·읽기 전용** 명문화(감사 무결성) · (2) **getAdminConfig 페이지네이션 없음**(device.*는 gw/1.1+ fleet 규모로 증가·policy와 동일) → limit/cursor 추가 · (3) **DELETE 감사 action 부재**(PUT=config.publish만 有) → `config.delete` 추가(db-jsonb 표준목록+엔드포인트 주석). **관찰(무변경)**: PUT-upsert 200-only(201 없음)=PUT 멱등 관례상 수용 · scopeId 조건부 필수=앱레벨 · version optimistic-concurrency=향후 enhancement. redocly valid·op 54·DBML OK | (작성자 ID 미지정) |
| 2026-07-10 | **`/v1/admin/audit` API 리뷰(감사 로그·FR-AUD-01·IEC 62304/ISO 13485)** — audit_log 조회 API 대조(OpenAPI↔DBML↔db-jsonb). **정합 확인**: append-only·조회 전용(write API 없음·GW 런타임 생성)·`result` enum{success,denied,failure} 일치·`action` free-string(db-jsonb 레지스트리)·beforeState/afterState nullable·PHI/secret 금지·필터(actor·action·result·from/to)+limit/cursor가 DBML 인덱스(ts·actor·action)와 정합. **정합 확인(추가)**: AuditLog 필드 8개(id·ts·actor·action·result·beforeState·afterState·**sourceIp**) ↔ DBML audit_log 완전 매핑(sourceIp 이미 존재). **🟡 수정**: **`required` 미선언** — DBML NOT NULL인 id/ts/actor/action/result → `required` 추가(클라 존재 보장). **관찰(무변경)**: `result` 필터는 인덱스 없으나 저카디널리티(3값)라 수용 · sourceIp 필터 미제공(actor가 주축·인덱스 없음). redocly valid·op 54·DBML OK | (작성자 ID 미지정) |
| 2026-07-10 | **클라이언트 SW 인벤토리 API 리뷰(`/v1/admin/clinics/{clinicId}/clients`·§7.8.5·FR-FLEET-06)** — client_inventory 조회 API 대조(OpenAPI↔DBML↔SRS↔matrix·③-C). **정합 확인**: `ClientInventory` 8필드↔DBML(surrogate `id`만 API 미노출=config처럼 natural-key·깔끔)·`required`=DBML NOT NULL 정합(clinicId·product·swVersion·firstSeen·lastSeen)·read-only(관측·Redis throttle async upsert·write API 없음)·튜플 식별(대수 아님)·pagination. **🟡 수정**: (1) **캠페인용 전 클리닉 횡단 조회 API 부재** — §7.8.5·③-C가 '특정 버전 미만 클리닉 전체' 가시성을 약속하나 per-clinic(`/clinics/{clinicId}/clients`)만 존재 → **`GET /v1/admin/clients`(횡단·product·clinicId(선택)·staleOnly·커서) 신설**(op 54→55·드릴다운은 기존 유지·semver '미만' 비교는 결과에 대해 Console) · (2) **client_inventory unique 튜플의 nullable `os`** — 미보고(null) 시 표준 UNIQUE 중복 미차단→throttle ON CONFLICT 불발·행 누적(policy/config/operator_role과 동일 NULL-distinct) → DBML 인덱스에 caveat + **os 미보고 시 sentinel 정규화 권고**(os가 identity+throttle 해시에 참여). **관찰(무변경)**: id 미노출 natural-key(config와 동일 모범)·staleOnly 임계=config/LLD. redocly valid·op 55·DBML OK. api-surface 헤더·Appendix B #6 카운트 54→55 동기화 | (작성자 ID 미지정) |
| 2026-07-10 | **Public plane API 리뷰(auth/token·enroll/start·enroll/complete·webhooks/{target}·well-known) — 흐름 계약** — OpenAPI↔SRS(§7.1·§7.2·§7.6·§7.7)↔redis 대조. **정합 확인**: securitySchemes 3종(deviceAuth·operatorAuth·webhookHmac) 명확·auth/token=public private_key_jwt(ADR-13·refresh 없음)·webhooks=HMAC+timestamp·target별 host 식별·202 ACK·eventId 멱등(redis wh:dedup)·envelope 예시(target 원 스키마 유연 §4.1.3)·DeviceStatus enum 정합. **🔴 수정(흐름 갭)**: **enroll start↔complete 상관 식별자 부재** — redis 키는 `gw:nonce:enroll:{challengeId}`인데 EnrollChallenge가 `challengeId`를 반환 안 하고 EnrollCompleteRequest도 안 보냄 → GW가 complete에서 저장 nonce·부트스트랩(Clinic-ID)을 조회할 방법이 없어 서명검증·clinic upsert 불가 → **EnrollChallenge/EnrollCompleteRequest에 `challengeId` 추가**(required). **🟡 수정**: TokenResponse `required`[accessToken,expiresAt] 선언 · well-known 200 응답 `content: application/json`(schema+design/well-known 포인터) 선언. **관찰(무변경)**: enroll 공개엔드포인트의 401(부트스트랩/서명 거부 표현·수용) · TokenResponse는 OAuth2 표준(access_token/expires_in) 대신 GW 자체 명명(의도). redocly valid·op 55·DBML OK | (작성자 ID 미지정) |

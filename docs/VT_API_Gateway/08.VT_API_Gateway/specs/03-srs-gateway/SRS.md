...
| 6 | OpenAPI·DBML 등 설계 산출물 | **작성 완료·검증 통과(2026-07-08)**: OpenAPI 41 ops(redocly valid)·DBML 10 테이블(dbml2sql OK)·redis 키스페이스·well-known 샘플. **잔여=baseline 후 code-first(@nestjs/swagger)로 정본 승계·example 완성**(자연 과정·결정 아님·dev-chain) | §1.5·§4.1·§6.4 |
| 47 | clinic·device API 분리 유지(통합 기각) | **확정(2026-07-08): device·clinic을 별도 리소스/API로 유지, device로의 통합 기각.** 사유=SoT(clinic=LMP 32자 / device=GW UUIDv7)·생성경로(clinic은 주로 enroll 자동·admin POST는 교정 예외 / device=enroll+C/S 승인)·lifecycle(device만 pending→…→revoked·kill)·관계역할(clinic=정책/config scope 앵커·org_mapping 라우팅 키·region 원천)이 모두 상이. **현 1:1은 배포 우연이지 모델 제약 아님**(모델=device 주체·clinic 선택적 그룹·**1:N**·clinic-less #33·#19). 통합 정당조건(영구 1:1 ∧ 동일 SoT ∧ 동일 lifecycle ∧ 독립 참조 없음) **전부 불성립**. **중복감 해소=API 통합이 아니라 Console UI(③-C)에서**: device-중심 마스터-디테일 + **편집면 단일화**(region·org-binding=clinic 화면만·lifecycle=device 화면만) + 양방향 링크. 선택 후속(관계 일급화)=device 응답에 clinic 요약 임베드·`GET /v1/admin/clinics/{clinicId}/devices` 하위목록. UI 구성 씨앗=③-C `_status` | §1.2·§7.2·§7.3·③-C |

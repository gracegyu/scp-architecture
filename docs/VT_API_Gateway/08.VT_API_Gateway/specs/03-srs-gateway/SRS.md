...
| # | 항목 | 본문 | 책임자 | 마감 | 영향 |
| --- | --- | --- | --- | --- | --- |
| 32 | **정책(policy) 관리 API + Console UI** — **관리 API 신설 완료(2026-07-07)**: `/admin/v1/policies` GET(scope·target 필터)·POST(upsert)·DELETE(§7.9.1·OpenAPI). deny-by-default라 **v1.0 필수**(허용 정책 없으면 프록시 전면 거부). **v1.0 집행=coarse 확정**(upstream 사용 허용까지; 작업·데이터 격리는 AXS+Org-ID 위임) — `allowed_endpoints`·`scopes`·토큰 `scope` 세분화는 **스키마만 유지한 예약**, gw/1.1+ 활성화(클리닉별 차등·침해 device blast-radius·다중 upstream 시). 잔여=**Console UI(③-C)** + 세분화 시 차원별 병합 규칙(OPA/Rego·LLD). 스키마·jsonb=design/db-jsonb-fields.md. **정책 스코프 = device-중심(device→clinic→global) 확정**(2026-07-06, §1.2·§6.4.1 — 구 'clinic 단위·device 배제'를 대체). **스코프 평가 규칙(deny-by-default·clinic=상한) = §7.5.3 확정**; 차원별 병합(교집합/우선)만 OPA(Rego)/LLD 잔여 | §7.5.3·§7.9·§6.4·design/db-jsonb-fields.md | GW(관리 API)+③-C(UI) | 정책 구현 착수 전 | §7.1·③-C Console Sub-SRS |
...
| 30 | **region 카탈로그 관리 API + Console UI** — **관리 API 신설 완료(2026-07-07)**: `/admin/v1/regions` POST(개통)·`/{regionId}` PUT(active/draining/planned 전이)·DELETE(회수), 조회는 `GET /v1/regions`(§7.3.6·OpenAPI). 잔여=**Console region 관리 UI(③-C)**. v1.0=1행 시드로 충분·gw/1.2 다행 | §7.3.6·§7.9·§6.4 | GW(관리 API)+③-C(UI) | gw/1.2 멀티리전 착수 전 | §7.3·③-C Console Sub-SRS |

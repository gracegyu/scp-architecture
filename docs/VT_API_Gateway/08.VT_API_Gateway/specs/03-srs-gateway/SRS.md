...
- 조회 API = **`GET /v1/admin/clinics/{clinicId}/clients`**(product 필터·staleOnly·커서). EzServer는 `GET /v1/admin/fleet`.
...
| 6 | OpenAPI·DBML 등 설계 산출물 | **작성 완료·검증 통과(2026-07-08)**: OpenAPI 54 ops(redocly valid)·DBML 13 테이블(dbml2sql OK)·redis 키스페이스·well-known 샘플. **잔여=baseline 후 code-first(@nestjs/swagger)로 정본 승계·example 완성**(자연 과정·결정 아님·dev-chain) | §1.5·§4.1·§6.4 |

...
- 범위: **단일 Region GW(3단계·v1.0)** — **Route 53 GeoDNS 라우팅·DNS 호스트(apex `gw.vatech.com` + webhook `{provider}.webhook.gw.vatech.com`)·K8s HA·고정 egress IP를 v1.0부터 구축**(GeoDNS 대상=서울 1개로 resolve, 멀티리전-ready, SRS §2.7.1·§4.5.1). **4단계=N리전 활성화**(GeoDNS 라우팅 대상 증분·글로벌 복제 — record 타입·클라이언트 변경 없음). 비-AWS국 MinIO는 Provider 제공(GW 중계만)

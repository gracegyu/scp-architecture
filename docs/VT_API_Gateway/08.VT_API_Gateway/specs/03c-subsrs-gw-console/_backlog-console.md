...
| `T-FE-8-2` staging 실연동 | **⚠ staging 을 세울지 자체가 미결** · staging GW 배포 · **C-3** CORS | **회의·③-I** |
| `T-FE-8-3` baseline 승인 | 사람 판단(위 A-3) | PL |
| `T-FE-8-4` prod 배포 | **C-10** 도메인 확정 · **BLOCKER** | 회의·③-I |

> ⚠ **2026-08-19 — `T-FE-8-2` 의 선결이 "배포 대기"가 아니라 "존재 여부 미결"이었다.** 부모 GW SRS **§3.1** 은 dev/staging/prod 3계층을 정의하고 **§3.5** 는 *"staging 에서 통합·E2E·부하·인수 검증을 수행"* 한다고 적는데, **`infra-requests.md` 에 staging 언급이 0건**이다(실측) — 아무도 ③-I 에 만들어 달라고 하지 않았다. `T-FE-8-2` 는 "staging GW 배포"를 선결로 적어만 두고 있었다.
>
> **Console 만의 문제가 아니다** — GW **12-3(부하 실측)·12-4(HA)** 도 같은 staging 을 기다린다. → **"12-3/12-4 가 v1.0 범위인가"가 정해지면 staging 필요 여부도 사실상 정해진다.**
>
> **결정이 필요한 것**: v1.0 에서 전용 staging 을 세울 것인가, dev 하나로 갈 것인가. 범위·예산 사안이라 **회의 결정**이며, 스펙 세션이 Raymond 에게 상신했다(2026-08-19).
> - **세운다** → §3.5 현행 유지(계층 구분만 추가) + **③-I 에 staging 프로비저닝 요청 신규 필요**
> - **dev 만** → §3.5 의 "staging" 이 실체 없는 계층을 가리키게 되어 스펙 정정 필요 · 공유 dev 에는 **자동 e2e 를 두지 않고** 개통 스모크 3~5 + `[manual]` 1회
>
> ⚠ **연동**: GW `seed-guard` 를 dev RDS 까지 넓힐지 → "dev 에서 리셋을 돌리나" → 이 결정. **세 개가 한 덩어리라 따로 정할 수 없다.** 상세 = `tests/e2e/staging/README.md` 머리말.
>

...
- 상태: **`SRS.md` v0.10 — 부모 Admin API 커버리지 감사(26/26)까지 반영·baseline 동결 가능**(2026-08-05·Case C·부모 spec-v1.0.10). v0.10=부모 OpenAPI `/v1/admin/*` 26개 전부 FR-CON 매핑 확인 + L2(FR-CON-37)·config 경로 인용 정리. v0.7=FR-CON-36 stale-write 용어 일관성 정정(낙관잠금=클라 감지로 하향·서버강제는 Appendix C-11 권고). v0.8=§5·§6·§4 구조(제목/본문) 분리·§1.2 결정 블록쿼트 제거·§4.2 화면 맵 추가·§6.10 국제화 확정(표준 PO+소스 영어원문·심볼 키 금지 → LinguiJS 선정·cloudwebviewer 레퍼런스). v0.9=재검증 지적 반영(H1 §4.2 화면 맵 FR-CON 그룹핑 정정·M1 접근성 참조 분리·L1 헤딩 빈 줄). 본 `_status.md`는 씨앗(근거), 정본=`SRS.md`. 미결=`SRS.md` Appendix B(C-1~C-13·전부 소유자·시점 명시된 정상 추적 항목·baseline 비차단)·Decision Log=Appendix A(A~E)·변경 이력=문서 말미. **다음 단계 = 사장님 검토 → (원하면) baseline 동결·PR·console repo 이관.**
- 문서 유형: Sub-SRS

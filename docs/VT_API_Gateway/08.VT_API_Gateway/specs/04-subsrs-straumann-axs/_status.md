...
- TBD (④ baseline 시 확정 — **시나리오가 flow·API를 정한다**):
  - **상태 A/B/C 실재·분포** — 호주 실사용 클리닉이 이미 Vatech 연동(link·APPROVED)돼 `organizationId` 보유(A)인지, 대부분 미연동(`customerNumber`만·B)인지, **둘 다 발생**하는지 → **주간회의 7/9 R8 확인**(AXS=호주 요구·기존 Straumann 계약 클리닉 존재 추정).

...
| **1. 투자 판단** | 호주 시장에서의 사업적 가치(장비 판매 기여도) 대비 총 개발/운영 비용(ES 개발 인력 3~4개월, AWS 월 $45~150)을 정량적으로 평가하여 진행 여부 결정 | VT 경영진 | 0단계 완료 |
...
1. **클라우드 개발 기간 대폭 단축**: 자체 Auth Server 2~3개월 → API Gateway 2~4주. 단, EzServer 측 개발(FE/BE/SRS/MMI)은 별도 2~3개월 소요
...
운영 안정성을 위해 AWS 인프라는 다중 환경으로 구성해야 한다:

| 환경 | 용도 | 비고 |
|------|------|------|
| **dev** | 개발/단위 테스트 | Straumann 샌드박스 연동 |
| **staging** | 통합 테스트/QA | 프로덕션과 동일 구성 |
| **prod** | 운영 환경 | 실제 클리닉 트래픽 처리 |

월 운영 비용($15~50)은 **환경당** 비용이며, 3개 환경 운영 시 월 $45~150 수준이 된다. 다만 dev/staging은 상시 트래픽이 없으므로 실제로는 prod보다 낮은 비용이 발생한다.

### EzServer 개발 공수

위의 API Gateway/Lambda는 **클라우드 측 개발**에 해당한다. AXS 연동을 완성하려면 **EzServer 측 개발**이 별도로 필요하며, 오히려 이쪽의 공수가 더 크다.

#### EzServer 개발 항목

| 항목 | 내용 |
|------|------|
| **SRS 작성** | AXS 연동 기능에 대한 소프트웨어 요구사항 명세 |
| **MMI 업데이트** | 의료기기 소프트웨어 변경에 따른 문서 갱신 |
| **Frontend (Web Console)** | AXS 연동 설정 UI, Organization-ID 관리, 연동 상태 표시 등 |
| **Backend (PMS Integration)** | 환자 데이터 추출, AXS API 호출 로직, Pre-signed URL 업로드 처리 등 |

#### 전체 개발 일정 추정

| 영역 | 예상 기간 | 비고 |
|------|----------|------|
| API Gateway + Lambda (클라우드) | 2~4주 | Straumann API 스펙 수령 후 착수 |
| EzServer (SRS + FE + BE + MMI) | **2~3개월** | 클라우드와 병렬 개발 가능 |
| **통합 테스트** | 추가 2~4주 | **양쪽 모두 개발 완료 후** 가능 |

- 클라우드와 EzServer는 **병렬 개발**이 가능하나, **통합 테스트는 양쪽이 모두 완료되어야** 시작할 수 있다.
- 따라서 전체 일정은 EzServer 개발 기간에 좌우되며, 총 **3~4개월** 이상 소요될 것으로 예상된다.

#### Estimation의 한계

현재는 Straumann으로부터 AXS API 스펙, 인터페이스 명세, 연동 가이드 등을 받지 못한 상태이다. 위 일정 추정은 **유사 연동 경험 기반의 개략적 추정**이며, 정확한 Estimation은 다음이 확보된 이후에만 가능하다:

- AXS API 전체 엔드포인트 및 데이터 모델 명세
- 연동 시나리오별 시퀀스 (환자 생성, 영상 업로드, 케이스 동기화 등)
- EzServer FE/BE에서 처리해야 할 구체적 기능 범위

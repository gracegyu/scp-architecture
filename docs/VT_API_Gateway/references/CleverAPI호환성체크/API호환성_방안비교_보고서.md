...
**해결 방향.** 한 제품만 고쳐서는 풀리지 않는다. **서버**(CleverSpace)가 호환 기준을 정해 제약을 집행하고, 클라이언트는 그 기준을 미리 확인하도록 역할을 나눠 **2단계**로 해결한다.
...
**투자 대비 효과.** 단계 구분은 **최소 투자로 최대 효과**를 노린 가성비 전략이다. 1차는 **기존 자산(서버 검증 경로·릴리즈 프로세스)을 재사용**해 적은 공수로 silent failure·매출 손실·CS 부담을 즉시 줄이고, 고투자 구조 항목(게이트웨이)은 효과 검증 후 2차로 단계 집행한다(상세 §3.2).
...
> **방법 1·2와의 차이:** 방법 1·2가 런타임에 "확인하고 막는" **행동**이라면, 방법 4는 그 행동이 참조하는 **기준 데이터**(어떤 API가 어느 버전부터 되는지 적은 표)다. 즉 방법 4는 혼자서 무엇을 막지 못하고, **방법 1·2가 판단할 근거를 제공**한다.
...
완벽한 해법(모든 경로·모든 구버전 일괄 통제)은 구조 변경·전 클라이언트 재배포를 수반해 **투자가 크다.** 기업 현실상 **최소 투자로 최대 효과**를 먼저 거두고, 큰 투자는 효과를 검증한 뒤 단계적으로 집행하는 것이 합리적이다. **1차/2차 분리 자체가 이 가성비 전략**이다.

- **1차(런타임 보완):** 이미 있는 자산을 재사용해 **적은 공수로 silent failure를 즉시 차단**한다. — 서버의 기존 EPI 검증 경로 활용(방법 2), `MessagingDialog`의 하드코딩 분기를 registry로 대체(오류 코드), 릴리즈 프로세스에 매트릭스 편입(방법 4).
- **2차(구조 개선):** Gateway·well-known처럼 **효과는 크지만 공수도 큰** 항목은, 1차로 급한 불을 끈 뒤 **장기 투자**로 단계 집행한다(방법 3).

**방법별 공수 × 효과**

| 방법 | 공수(투자) | 효과 | 가성비 | 핵심 근거 |
|------|-----------|------|--------|-----------|
| **방법 2** 서버 주도 검증 | **낮음** (서버 1곳 수정) | **높음** (전 클라이언트 즉시 적용) | ★★★ | 기존 EPI/`validate-limits` 경로 활용. 구버전 재배포 불필요 |
| **방법 4** 호환성 매트릭스 | **낮음** (문서+CI) | 중 (다른 방법의 데이터 소스) | ★★★ | 릴리즈 프로세스 편입. 단일 소스→자동 생성(§C.2.1)로 운영비 최소 |
| 오류 코드 registry+fallback | 낮음 (파일 1벌) | 중~높음 (silent failure 제거) | ★★★ | 하드코딩 switch 대체, 신규 코드 추가만으로 일관 처리 |
| **방법 1** 클라이언트 사전 조회/내장표 | 중 (N개 클라이언트 배포) | 높음 (UX 선제 차단) | ★★ | UX 가치 크나 구버전은 갱신 한계. 신규 버전부터 효과 |
| **방법 3** EzServer Gateway | **높음** (구조 변경·인증 통합) | **최고**(장기) (경로 B 흡수·통제 일원화) | ★(단기) / ★★★(장기) | 투자 크므로 2차에서 단계 집행 |

> 정리: **저공수·고효과는 방법 2·4 + 오류코드 registry**(=1차 core), 여기에 UX를 더하는 **방법 1까지가 1차 권장**(2+4+1)이다. 고투자 구조 항목(방법 3)은 효과가 검증된 2차로 미룬다.

**“하지 않을 때”의 비용(투자 정당화 근거)**

가성비는 들이는 비용뿐 아니라 **방치 비용**과 비교해야 한다. 현행 유지 시:

- 신규 **유상 기능이 구버전에서 조용히 미동작** → 매출 기회 손실, 기능 도입 효과 저하.
- 원인 안내 없는 실패(업로드·공유 거부 등) → **CS 문의·클레임 증가**, 임상 현장 신뢰 저하.
- 경로 B 인증·통제 분산 방치 → 보안·운영 리스크 누적, 나중에 더 큰 비용으로 전가.

즉 1차 항목들은 **낮은 투자로 위 손실을 즉시 줄이므로 가성비가 가장 높고**, 2차는 반복되는 구조적 비용을 **근본 제거**하는 장기 투자다.

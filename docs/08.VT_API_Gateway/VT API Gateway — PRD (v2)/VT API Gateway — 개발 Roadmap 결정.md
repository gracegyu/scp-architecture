...
본 Roadmap은 아래 구성으로 **OnePager(프로젝트 단위 스펙)** 를 작성한다(본 보고서 작성 시점에는 미작성, 추후 작성). **케이스 D 확정**(1·2 병행 + 3·4·5 통합)에 맞춰, 스펙 경계를 **실행 구조에 맞게** 나눈다 — 병행 트랙(1·2)은 각각 독립 OnePager로, GW 플랫폼(3·4)은 한 스펙으로, Straumann(5)은 외부 의존·리스크가 달라 별도 스펙으로 둔다.

| OnePager | 범위 | 비고 |
|----------|------|------|
| ① API 호환성 | 1단계 | 긴급·독립(v1.3.0 대응), 2와 병행 |
| ② presigned 데이터 경로 | 2단계 | GW 일원화의 선행 요건, 1과 병행하는 단독 산출물(CleverSpace presigned 발급·EZ 전송 로직·업로드 완료 확인) |
| ③ VatechAPIGateway 구축 | 3+4단계 | GW 일원화 + 멀티 Region. 4는 3의 확장 속성이라 한 스펙. 일정에 단계별 구현·병행/통합 기술 |
| ④ Straumann(AXS) 연동 | 5단계 | 갈래 A(EzServer→AXS)·갈래 B(CleverLab↔AXS) 포함. 외부 이해관계자·일정 리스크가 상이해 별도 스펙 |

> 스펙 경계 ≠ 실행 경계다. 케이스 D는 ③(3+4)과 ④(5)를 **통합 실행**하지만, 외부 의존이 다른 Straumann은 **승인·협의 단위로 OnePager를 분리**해 둔다. ①·②는 병행 트랙이라 각각 독립 스펙으로 둔다.
